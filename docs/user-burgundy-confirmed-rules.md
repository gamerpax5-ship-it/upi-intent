...
- Draft branch `wpay/user-workflow`: last pushed commit `d7ed39d82b3fd51f0d93632a0b3aef26e356eecc`, tree `a4fe5c74390af5f25b2d89408e79666b28a78b61`. Production stays at `ed68f8b612912e835987d79113d7a5c1e3ed247c`.
- Migration 025: optional payout source bank, system-attributed timeout settlement. Migration 026: audited free-setup/unlimited-collection permissions and independent UPI daily limits. Neither applied to production.
- Implemented in draft: Burgundy shell/logo, dashboard real account aggregates and charts, date-only pay-in table, historical payout commission with first-effective-rate fallback for newly registered users, one active payout and Parking lock, 10+5 minute submission windows, payout review timeout, Parking minimum/maximum, first-deposit minimum, manual deposit approval without hash and subsequent provider deduplication, fresh-bank withdrawals, gross-inclusive 0.5% INR fee, rounded USDT full-balance debit, visible withdrawal reasons, per-UPI IST daily limit editing, Admin free/unlimited controls, capacity-building payout/Parking routing, positive-capacity setup gate. Existing OTP event method and protected legacy files unchanged.
- Isolated CI runs 36182481891 and 36183371996 passed. Run 36184333197: 49 focused DB/regression checks passed; full npm suite 532/538 passed, 6 failed (including a parent test failure), zero skipped. New database tests cover historical rates, manual deposits/replays/minimum, new-bank withdrawals/fees/FX/concurrency, free access revocation, separate UPI limits, dashboard SQL.
- Failures diagnosed: two obsolete tests hardcode latest migration 19 despite baseline already having 24; route funding was too broad (local fix now accepts approved deposit or completed payout/Parking provenance), an existing gateway no-route test expected no automatic diagnostic request (local fix makes diagnostic user-triggered), reference HTML serialization/favicon contract (local fix with hidden attribute and real WPay SVG asset). Eight local reference/payment-link tests now pass. User dashboard DOM check passes.
- The existing tests are untouched per AGENTS.md. A reviewable patch for the two obsolete version assertions is required before full-suite success can be claimed. Do not silently weaken checks or alter migration behavior to satisfy stale test numbers.
- STILL INCOMPLETE: exact inner-page prototype UI beyond dashboard/pay-in shell; Business UPI versus Merchant QR form split; 48h post-approval disputes, late-proof review, full random UPI verification and device-health rules, 24h pairing/code revoke/history/role scopes, APK update/permission/location features, all corresponding Admin/Employee pages. Do not call the task complete or deploy this intermediate branch.
- After last remote push, local code has additional route provenance, favicon, page-load race/duplicate heading, date UI and gateway diagnostic fixes. Preserve all local files; compare against remote before publishing next checkpoint.
