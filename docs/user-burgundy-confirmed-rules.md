...
- Draft branch wpay/user-workflow created at 99188970ad866bba22f64721cf1e8bb5accd7b43; isolated CI 36182481891 passed. Production remains unchanged. Additional local work after that commit: fresh-bank withdrawals, 0.5% gross-inclusive INR fee and net display, fractional-paise USDT debit rounding, manual deposit approval without hash with replay protection, full User dashboard aggregates and Burgundy dashboard rendering; dedicated database tests are being added/run. Other inner pages, disputes, late-proof review, full APK/device policies and remaining Admin controls are still incomplete. Preserve local changes and inspect diff.
