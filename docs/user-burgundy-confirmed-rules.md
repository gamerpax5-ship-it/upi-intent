...
- Draft branch `wpay/user-workflow`; current tested commit `bc8ed000cfac354a3bffafef3a8cc6388810b7e7`, tree `865f71e994cf201b1bd30074c0835a4364eb1c18`, draft PR https://github.com/gamerpax5-ship-it/upi-intent/pull/19. Production stays at `ed68f8b612912e835987d79113d7a5c1e3ed247c`.
- Migrations 025–027 add optional payout source bank/system timeout settlement, audited free setup/unlimited collection and independent UPI daily limits, append-only post-approval payout disputes. None applied to production.
- Implemented in draft: Burgundy shell/logo, dashboard real account aggregates and charts, date-only pay-in table, historical payout commission with first-effective-rate fallback, one active payout and Parking lock, 10+5 minute submission windows, payout review timeout, Parking min/max, first-deposit minimum, reason-only manual deposit approval and provider deduplication, fresh-bank withdrawals, gross-inclusive 0.5% INR fee, rounded USDT full-balance debit, visible withdrawal reasons, per-UPI IST daily limits, Admin free/unlimited controls, positive-capacity setup gate. Admin access navigation filtering fixed. Existing OTP event method and protected legacy files unchanged.
- Added post-approval disputes: Merchant 48-hour window, fresh receiving statement plus declared coverage/reason, immediate capacity/commission hold, User explanation/additional proof, scoped Admin/Employee review page, valid-payment release, invalid-payment exact original settlement reversal and full Merchant principal/fee refund. Original terminal payout/settlement remain immutable; history projects reversed status. Retries/final decisions cannot post twice. Negative capacity/commission deficits remain after already-spent credits. Reviewer must inspect statement contents; upload itself never proves payment.
- CI run 36187792557 at bc8ed000: 66 focused DB/regression checks passed, zero skipped; complete DOM regression step passed, including Merchant dispute upload. Full npm suite 540/542 passed; only 2 failures are old migration assertions expecting version 19 (actual 27). New dispute tests exercised concurrent duplicate opening, ownership, responses, finality, holds/releases, negative balances, two original capacity paths, Merchant refund, scoped denial and immutable rows. Existing tests untouched. Local User dashboard and Merchant payout DOM checks passed; protection checker passes 197 frozen hashes.
- Latest tested additions: exact 48h boundary test, resolution notifications, routing provenance excludes invalidated payouts, resolved dispute UI no longer labels released money as held, User Holds visibility and Merchant ledger reversed status. Protection check 197/197, syntax checks 133 files. No production deployment.
- Reviewable, unapplied patches: `docs/proposed-migration-test-update.patch` changes exactly two expected migration numbers 19 -> 27. `docs/proposed-pairing-24h-issuer.patch` proposes only the protected legacy pairing issuer's new-code expiry and response 600 -> 86400 seconds. AGENTS.md requires explicit approval for existing-test/protected-file edits. Do not apply or update protection baseline silently.
- Pairing inspection: current local legacy issuer hardcodes 10-minute codes; both pairing adapters require 600-second responses. A dashboard-only 24h label cannot implement 24h validity. Actual second Railway service issuer must also be identified and verified. No Railway or OTP changes made.
- STILL INCOMPLETE: exact inner-page prototype UI beyond dashboard/pay-in shell; Business UPI vs Merchant QR form split; late-proof review; full random UPI verification/device-health rules; 24h pairing/revoke/history/Admin-Employee issue scopes; APK update/permission/location features; corresponding remaining Admin/Employee interfaces; live authenticated inspection and deployment. Do not claim complete or deploy this intermediate branch.

## Follow-up review — 2026-09-26 IST

- Added a separate DeviceSetup backend and new metadata-only endpoints. Existing Devices.events, operational OTP source and legacy files remain unchanged. User permissions no longer need OTP permission to view the new setup UI. Admin/Employee view, issue and unlink actions use explicit device permissions and tenant scope.
- Added account-owned Admin/Employee code issuance, trusted pairing-proof ownership, idempotent code issuance, automatic browser status checks, code history pagination and device cards with actual version/heartbeat status. History does not expose code/token/credentials. Migration 028 grants new management permissions only to Super Admin; Admin/Employee delegation remains explicit.
- Pending device work remains: actual issuer 24h expiry (currently still 10m), unused-code revocation at source, background pairing sync without an open page, richer diagnostics/location/permission health and Android update flow. Missing metadata is unavailable, never fabricated.
- Pending other work remains: inner prototype pages and Business UPI/Merchant QR split; late-proof review; complete random-amount UPI verification with online APK, 10+5-minute windows and trusted SMS/statement evidence; live deployment and authenticated visual inspection.
- Existing-test patch now proposes exactly the two stale assertions 19 -> 28. It and the protected pairing-issuer patch remain unapplied; no protection baseline changes.
