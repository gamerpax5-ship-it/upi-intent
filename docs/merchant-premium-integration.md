...
CI on d796553 reported two pre-existing schema-version assertions in `test/wpay-live-user-merchant.test.js` (lines 41 and 54): expected 19, current schema 22. This change does not modify migrations or those tests. They remain enabled and failing visibly; DOM checks run independently even after a failure. Do not describe the full CI suite as green.
