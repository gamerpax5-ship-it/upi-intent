...
async function rolesAndResources(){
  ...
  try{
    ...
    for(const database of ['wpay_9a_admindelegation_0','wpay_9a_internalcompletion_0','wpay_9a_upgradeinternal_0','wpay_15_upgrade_internal_ci','wpay_9a_passwordpolicy_0'])await client.query('CREATE DATABASE '+database+' OWNER wpay_migrator');
    ...
    for(const [name,file] of [['passwordpolicy','wpay-password-policy'],['admindelegation','wpay-admin-delegation'],['internalcompletion','wpay-internal-completion'],['upgradeinternal','wpay-upgrade-016']]){
