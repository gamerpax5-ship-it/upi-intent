...
async function main(){
  ...
  const pool=createHostedPool();let stopping=false,source;
  try {
    ...
    const server=await startAuthServer({service:new AuthService(new SecurityRepository(pool),{mfaCrypto,legacyReader:source.reader,fundingProvider:fromEnvironment(),fixedCurrency:process.env.WPAY_HOSTED_FIXED_FEE_CURRENCY}),
      port:Number(process.env.PORT),hostedOrigin:policy.origin,readiness:async()=>{
        if(stopping)return false;
        try {await pool.query("SELECT 1");return true;}catch{return false;}
      }});
    const stop=()=>{if(stopping)return;stopping=true;const deadline=setTimeout(()=>server.closeAllConnections(),10000);deadline.unref();server.close(()=>{clearTimeout(deadline);pool.end().catch(()=>{});source.close().catch(()=>{});});server.closeIdleConnections();};
