...
async function main(){
  ...
  const pool=createHostedPool();let stopping=false;
  try {
    ...
    const stop=()=>{if(stopping)return;stopping=true;const deadline=setTimeout(()=>server.closeAllConnections(),10000);deadline.unref();server.close(()=>{clearTimeout(deadline);pool.end().catch(()=>{});});server.closeIdleConnections();};
    process.once("SIGTERM",stop);process.once("SIGINT",stop);
    console.log("WPay hosted authentication listening; financial and device sources are not connected.");
    return server;
