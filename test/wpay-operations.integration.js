...
test('Task 12 real PostgreSQL, protected legacy pairing/OTP and scoped operational authorization',async t=>{
 ...
 await t.test('fresh migration and source role cannot read credentials or mutate legacy',async()=>{assert.equal((await owner.query('SELECT max(version) v FROM wpay_auth.schema_migrations')).rows[0].v,15);assert.equal(await legacy.source.ready(),true);await denied(legacy.reader.query('SELECT credential_hash FROM public.devices'),'42501');await denied(legacy.reader.query("UPDATE public.devices SET status='active'"),'42501');});
