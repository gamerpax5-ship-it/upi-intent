...
(function(root){
 ...
 async function render(o,state={}){
  ...
  if(pending){
   const label=el('label','Review status '),filter=el('select');for(const value of ['pending','verified','rejected','all']){const option=el('option',value[0].toUpperCase()+value.slice(1));option.value=value;filter.append(option);}filter.value=state.status||'pending';filter.onchange=()=>action(()=>reload({status:filter.value,offset:0}));label.append(filter);toolbar.prepend(label);
   const data=await post('operations/utr/pending',{status:state.status||'pending',offset:state.offset||0});
   card.append(el('p','Customer-submitted UTR claims awaiting review. Verify checks bank evidence now. Reject closes the whole payment as Failed.','notice'));
   const review=(r,mode)=>{
    ...
    dialog.append(el('h2',mode==='verify'?'Verify UTR':'Reject payment'),el('p',r.utr+' · '+r.reference+' · '+money(r.amountMinor)),form);
    if(mode==='reject')form.append(el('p','This rejects the payment and all of its submitted UTR claims.','notice'));
    const submit=el('button',mode==='verify'?'Verify now':'Confirm rejection');submit.type='submit';form.append(submit,button('Cancel',()=>dialog.close()));
   ...
   table(['Submitted','UTR','Reference / order','Merchant','User','Amount','Payment status','Review status','Actions'],data.records.map(r=>{const actions=el('div',undefined,'admin-row-actions');if(r.canReview)actions.append(button('Verify',async()=>review(r,'verify')),button('Reject',async()=>review(r,'reject')));return [new Date(r.submittedAt).toLocaleString('en-IN'),r.utr,r.reference+' / '+r.orderId,r.merchant,r.user,money(r.amountMinor),r.paymentStatus,r.status,actions];}));
   if(!data.records.length)card.append(el('p','No UTRs match this review status.','admin-empty'));
   const pager=el('div',undefined,'admin-pagination');if(data.offset)pager.append(button('Previous',()=>reload({...state,offset:Math.max(0,data.offset-50)})));if(data.hasMore)pager.append(button('Next',()=>reload({...state,offset:data.offset+50})));card.append(pager);return;
  }
