...
(function(root){
 ...
 root.WPayReferenceUi={
  ...
  settings(){const host=document.getElementById('page-content');document.getElementById('page-title').textContent='Settings';const card=document.createElement('section');card.className='card';const label=document.createElement('label');label.textContent='Appearance';const select=document.createElement('select');for(const value of ['dark','light']){const option=document.createElement('option');option.value=value;option.textContent=value==='dark'?'Dark':'Light';select.append(option);}select.value=document.documentElement.classList.contains('light')?'light':'dark';select.onchange=()=>{document.documentElement.classList.toggle('light',select.value==='light');try{localStorage.setItem('wpay-role-theme',select.value);}catch{/* Appearance still works without persistence. */}};label.append(select);card.append(label);const security=document.createElement('button');security.type='button';security.textContent='Authenticator & password settings';security.disabled=!nav.resolve(role,'security',groups);security.onclick=()=>this.select('security');card.append(security);host.replaceChildren(card);},
  select,
