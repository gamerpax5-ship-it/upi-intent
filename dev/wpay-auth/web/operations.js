 ...
 async function render(options,state={}){
  ...
  if(page==='devices'||page==='activation'){
   ...
   if(page==='activation'){
    ...
    for(const r of history.requests){const tr=el('tr');for(const value of [new Date(r.created_at).toLocaleString(),r.owner_name,r.state+(r.revoked_at?' · device unlinked':''),r.device_ref||'—',new Date(r.expires_at).toLocaleString()])tr.append(el('td',value));const actions=el('td');if(r.canCheck)actions.append(button('Check pairing',async()=>{await post('operations/device-setup/poll',{requestId:r.id});await reload();}));tr.append(actions);body.append(tr);}table.append(body);const scroll=el('div',undefined,'table-wrap');scroll.append(table);card.append(scroll);if(!history.requests.length)card.append(el('p','No activation codes issued yet.'));
   ...
   for(const d of data.devices){const row=el('article',undefined,'card pad route-card');row.append(el('h2',d.device),el('p',d.status, 'pill'));facts(row,{'Owner':d.ownerName,'APK version':d.apkVersion||'Unavailable','Last seen':d.lastSeenAt?new Date(d.lastSeenAt).toLocaleString():'Unavailable'});
    const detail=el('details'),summary=el('summary','Device details');detail.append(summary);facts(detail,{'WPay link':d.linked?'Linked':'Not currently confirmed','Access until':new Date(d.validUntil).toLocaleString(),'Diagnostics':'Only metadata supplied by the connected service is shown.'});row.append(detail);
