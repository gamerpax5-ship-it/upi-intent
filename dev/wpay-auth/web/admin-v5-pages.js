...
(function(root){
  ...
  async function supportPage(o){
    ...
    const rows=data.rows.map(t=>{const actions=el("div",undefined,"admin-row-actions");if(data.canWrite)actions.append(button(el,"Reply / update",()=>reply(t),"primary"));return [new Date(t.created_at).toLocaleString("en-IN"),t.subject,t.message,t.status,t.reply||"—",actions];});container.append(panelTable(el,["Created","Subject","Message","Status","Latest reply","Action"],rows));
