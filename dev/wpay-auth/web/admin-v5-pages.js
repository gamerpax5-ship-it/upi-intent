...
(function(root){
  ...
  async function employees(o){
    ...
    const rows=data.employees.map(e=>[e.name,e.email,pill(el,e.status),(e.admin_scope?.tenantIds||[]).join(", "),String(e.permissions.length)+" permissions",e.permission_version,data.canUpdate?button(el,"Edit",()=>editor(e)):"Read only"]);container.append(table(el,["Name","Email","Status","Tenants","Permissions","Version","Action"],rows));
    function editor(emp){
