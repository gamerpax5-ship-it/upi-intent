...
(function(root){
  ...
  async function payinDisputes(o){
    ...
    container.append(table(el,["Payment","Merchant","User","Amount","Reason","Fresh statement","State","Action"],rows));
  ...
  async function assignmentsPage(o){
    ...
    container.append(table(el,["Merchant","User","Priority","Amount range","User available","State","Action"],rows));
  ...
  async function devicesPage(o){
    ...
    container.append(metrics);
    const toolbar=el("div",undefined,"toolbar"),search=el("input"),status=el("select");search.className="control grow";search.placeholder="Search device, owner, number, SIM or network…";for(const [v,t]of [["","All status"],["online","Online"],["offline","Offline"],["unavailable","Unavailable"],["unpaired","Unpaired"]]){const op=el("option",t);op.value=v;status.append(op);}status.className="control";toolbar.append(search,status);container.append(toolbar);
    const grid=el("div",undefined,"device-grid");container.append(grid);
    const draw=()=>{const q=search.value.trim().toLowerCase(),st=status.value,rows=data.devices.filter(d=>(!st||d.status===st)&&[d.device,d.ownerName,d.phone,d.carrier,d.simName,d.network,d.model,d.apkVersion].join(" ").toLowerCase().includes(q));grid.replaceChildren();for(const d of rows){const card=el("article",undefined,"card device-card"),top=el("div",undefined,"device-top"),model=el("div",undefined,"device-model"),copy=el("div");copy.append(el("h3",d.model||d.device),el("p",(d.apkVersion||"APK unavailable")+" · "+(d.ownerName||"—")));model.append(copy);top.append(model,pill(el,d.status));card.append(top);const stats=el("div",undefined,"device-stats"),fact=(label,value)=>{const x=el("div",undefined,"fact");x.append(el("label",label),el("strong",String(value??"Unavailable")));return x;};stats.append(fact("Battery",d.battery==null?"Unavailable":d.battery+"% · "+(d.batteryHealth||"health unavailable")),fact("Network",(d.network||"Unavailable")+" · "+(d.carrier||"carrier unavailable")),fact("Location",d.locationEnabled===true?(d.locationLabel||"Enabled"):d.locationEnabled===false?"Disabled":"Unavailable"),fact("Valid until",d.validUntil?new Date(d.validUntil).toLocaleString("en-IN"):"Unavailable"));card.append(stats);const actions=el("div",undefined,"account-card-actions");if(!d.legacyMapping)actions.append(button(el,"View details",()=>detail(d)));if(data.canRevoke&&!d.legacyMapping)actions.append(button(el,"Unlink from WPay",()=>unlink(d),"danger"));card.append(actions);grid.append(card);}if(!rows.length)grid.append(el("div","No matching devices.","card admin-empty"));};
  ...
  async function merchantUsdt(o){
    ...
    container.append(table(el,["Merchant","INR reserved","USDT quote","Rate","Network / destination","State","Action"],rows));
    function editDefault(){dialog(el,container,"Edit default Merchant USDT rate",(body,d)=>{const form=el("form",undefined,"form-grid"),tenant=selectField(el,form,"tenant","Workspace",(defaults.tenants||[]).map(x=>[x.tenantId,x.tenantId]),defaults.tenants?.[0]?.tenantId),rate=field(el,form,"rate","INR per USDT",defaults.tenants?.[0]?.rate||"107"),save=el("button","Save default rate","primary");tenant.onchange=()=>{rate.value=defaults.tenants.find(x=>x.tenantId===tenant.value)?.rate||"107";};save.type="submit";form.append(el("p","This default pre-fills new Merchant approvals. Existing Merchant commercial versions are not silently rewritten.","notice"),save);body.append(form);form.onsubmit=e=>{e.preventDefault();action(async()=>{await post("panel/merchant-default-rate/update",{tenantId:tenant.value,rate:rate.value});d.close();await merchantUsdt(o);});};});}
  ...
  async function payoutDisputes(o){
    ...
    container.append(table(el,["Payout","Merchant / User","Amount / commission","Coverage","Reason","Proofs","Status","Action"],rows));
  ...
  async function directory(o,type){
    ...
    async function createAccount(){
      ...
      dialog(el,container,"Create "+(isUser?"User":"Merchant"),(body,d)=>{
        ...
        if(isUser){
          ...
          controls.payin=field(el,form,"payin","Pay-in fee %","1.20");controls.payout=field(el,form,"payout","Payout fee %","0.80");controls.fixed=field(el,form,"fixed","Fixed payout fee INR","6");controls.rate=field(el,form,"rate","INR per USDT",opts.defaultMerchantInrPerUsdt||"107");
        }
    ...
    async function approve(a,ok){
      ...
      dialog(el,container,(ok?"Approve ":"Reject ")+a.name,(body,d)=>{
        ...
        else{controls.payin=field(el,form,"payin","Pay-in fee %",settings.payinFee||"1.20");controls.payout=field(el,form,"payout","Payout fee %",settings.payoutFee||"0.80");controls.fixed=field(el,form,"fixed","Fixed payout fee INR",settings.fixedPayoutFee||"6");controls.rate=field(el,form,"rate","INR per USDT",settings.inrPerUsdt||opts.defaultMerchantInrPerUsdt||"107");}
