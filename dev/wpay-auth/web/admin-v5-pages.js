...
(function(root){
  ...
  async function upiAnalytics(o){
    ...
    const grid=el("div",undefined,"admin-columns"),utilCard=el("section",undefined,"card admin-panel"),health=el("section",undefined,"card admin-panel");utilCard.append(el("h2","UPI utilization"),el("p","Shared daily limit consumption","admin-subtitle"));for(const b of byUtil){const limit=BigInt(b.sharedLimit||0),spent=BigInt(b.used||0),p=limit?Number(spent*10000n/limit)/100:0,row=el("div",undefined,"admin-review");row.append(el("span",(b.details?.upiId||b.id)+" · "+b.owner_name),el("strong",p.toFixed(1)+"%"));utilCard.append(row);}health.append(el("h2","Route health"));for(const [l,v]of [["Running / available",data.running+" / "+data.available],["Frozen UPI",data.banks.filter(x=>x.frozen).length],["Active routes",activeRoutes],["Transactions",totalTx],["Successful",ok],["Verification pending",data.banks.reduce((n,b)=>n+Number(b.pending||0),0)]]){const row=el("div",undefined,"admin-profit-row");row.append(el("span",l),el("strong",String(v)));health.append(row);}grid.append(utilCard,health);container.append(grid);
