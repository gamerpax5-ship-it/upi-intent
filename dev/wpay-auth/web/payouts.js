...
(function(root){
 ...
 async function render(args){
  ...
  if(page==='orders'&&merchant){const summary=await request('payout/summary');facts(card,Object.fromEntries(['available','reserved','principal','fees','held'].map(k=>[k,format(summary[k])])));
   ...
   submit(bulk,'upload',async()=>{const result=await post('payout/bulk',{idempotencyKey:batchId,file:await file(fileInput)});if(result.errors.length){errors.textContent=t('bulkErrors')+' '+result.errors.map(e=>e.row).join(', ');return;}await render(args);});const template=button('template',async()=>{const result=await request('payout/template');download(result.name,Uint8Array.from(atob(result.data),c=>c.charCodeAt(0)));});card.append(el('h2',t('bulk')),el('p',t('bulkHelp'),'hint'),el('p','Available Merchant balance: INR '+format(summary.available)+' · the server rejects and rolls back any batch that exceeds it.','notice'),template,bulk,errors);
  }
  if(page==='jobs'){
