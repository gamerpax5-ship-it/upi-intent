...
(function(root){
 ...
 async function merchant(args){
  ...
  recent.append(el('h2','Recent Orders'));for(const o of (orders.orders||[]).slice(0,6)){const r=el('article',undefined,'role-dashboard-row');r.append(el('strong',o.reference),el('span',money(o.amountMinor)+' · '+o.status));recent.append(r);}if(!(orders.orders||[]).length)recent.append(el('p','No orders yet.','hint'));
 ...
 async function user(args){
  ...
  const [business,payout,banks,parking]=await Promise.all([
   request('business/summary'),request('payout/summary'),request('business/banks'),safe(()=>request('parking/orders'),{orders:[],history:[]})
  ]);
