...
(function(root){
 ...
 async function render(options,state={}){
  ...
  const facts=(node,record)=>{const admin=['admin','super_admin'].includes(account.accountType),dl=el('dl',undefined,admin?'admin-details':'facts');const append=(k,v)=>{if(v===undefined||v===null)return;if(admin&&v&&typeof v==='object'&&!Array.isArray(v)){for(const [sub,value]of Object.entries(v))append(k+' · '+sub,value);return;}const label=t(k),pretty=admin&&label===k?k.replace(/([a-z])([A-Z])/g,'$1 $2').replaceAll('_',' '):label;dl.append(el('dt',pretty),el('dd',admin&&Array.isArray(v)?v.join(', '):typeof v==='object'?JSON.stringify(v):t(String(v))));};for(const [k,v]of Object.entries(record))append(k,v);node.append(dl);};
