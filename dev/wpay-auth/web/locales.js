...
(function(root) {
  const dictionaries = {
    ...
    "zh-CN": {
      ...
      payinCommission:"收款佣金百分比", payoutCommission:"付款佣金百分比", inrPerUsdt:"固定汇率 — 每 USDT 对应的 INR", depositNetwork:"USDT 充值网络", depositAddress:"分配的 USDT 充值地址", payinFee:"收款费率百分比", payoutFee:"付款费率百分比", fixedPayoutFee:"每笔付款固定费用", fixedFeeCurrency:"固定费用币种", paymentLinkTtlSeconds:"支付链接有效期（秒，管理员设置）", currencyMissing:"尚未配置固定费用币种，无法批准商户。",
  ...
  for(const [key,values] of Object.entries({enable:['Enable authenticator','Включить аутентификатор','启用身份验证器'],disable:['Disable authenticator','Отключить аутентификатор','停用身份验证器']})) ['en','ru','zh-CN'].forEach((language,index)=>{dictionaries[language][key]=values[index];});
