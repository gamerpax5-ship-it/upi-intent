...
(function(root){
 ...
 function chart(el,box,rows,days){
  box.replaceChildren();const recent=rows.filter(r=>+new Date(r.paidAt||r.createdAt||r.created_at)>=Date.now()-days*86400000);
  if(!recent.length){box.append(el('div','No activity in this period.','empty'));return;}
  const buckets=new Map();for(const row of recent){const key=new Date(row.paidAt||row.createdAt||row.created_at).toISOString().slice(0,10);buckets.set(key,(buckets.get(key)||0n)+BigInt(row.amountMinor||row.amount_minor||'0'));}
 ...
 async function render(args){
  ...
  if(merchant){
   ...
   load('operations.transactions',()=>post('operations/transactions',{offset:0}),r=>{
    ...
    const successful=r.records.filter(x=>x.status==='successful');
    chart(el,chartBox,successful,7);query('.main-grid .card-head p').textContent='Successful pay-ins from the latest 50 orders · payment date · UTC.';
    for(const button of document.querySelectorAll('#overviewRanges button')){button.onclick=()=>{document.querySelectorAll('#overviewRanges button').forEach(b=>b.classList.toggle('active',b===button));chart(el,chartBox,successful,Number(button.dataset.range));};}
