...
async function initDeviceOtpTables(pool) {
  if (!pool) return;
  await pool.query(`create table if not exists device_otp_events (id bigserial primary key, event_hash text not null unique, device_id text not null references devices(id) on delete cascade, sender text, code_mask text not null, otp_length integer not null, message_masked text not null, source text not null default 'sms', sms_received_at timestamptz not null, created_at timestamptz not null default now())`); 
...
function createDeviceOtpRouter({ pool }) {
  ...
  const eventHash = sha256([req.device.id, sender || "", received.toISOString(), source, otpCode].join("\u0000"));
  ...
  router.get("/admin/device/:deviceId/otp-events", async (req, res, next) => {
    try {
      ...
      const events = await pool.query(
        select id,sender,code_mask as otp_code,                 otp_length,message_masked,source,sms_received_at,created_at            from device_otp_events           where device_id=$1           order by sms_received_at desc,id desc           limit 200,
        [deviceId]
      );
      res.json({
        deviceId,
