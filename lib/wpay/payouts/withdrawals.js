...
const {POLICY,inrWithdrawal}=require('../business/user-workflow-policy');
const binding=(kind,id)=>'wpay-withdrawal-'+kind+':'+id;
class Withdrawals{
 constructor(crypto){this.crypto=crypto;}
 project(r,detail=false){const out={id:r.id,currency:r.currency,amountMinor:r.amount_minor,entitlementMinor:r.entitlement_minor,state:r.state,statusLabel:r.currency==='USDT'&&r.state==='requested'?'Pending Admin Processing':r.state,createdAt:r.created_at,completedAt:r.completed_at,rate:r.currency==='USDT'?r.snapshot.settings.inrPerUsdt:null,rateVersion:r.snapshot.version,manualProcessing:true,blockchainConfirmed:false,feeMinor:r.snapshot.withdrawal?.feeMinor||'0',netMinor:r.snapshot.withdrawal?.netMinor||r.amount_minor,reason:r.latest_reason||null};if(detail){out.destination=JSON.parse(this.crypto.open(r.encrypted_destination,binding('destination',r.id)));if(r.encrypted_completion)out.completion=JSON.parse(this.crypto.open(r.encrypted_completion,binding('completion',r.id)));}return out;}
 async request(client,userId,body){
  fields(body,['idempotencyKey','currency','amountMinor','bankId','bankDetails','network','address']);v.reference(body.idempotencyKey);if(!['INR','USDT'].includes(body.currency)||money.minor(body.amountMinor)>money.MAX)fail();
  if(body.currency==='INR'&&(body.network!==undefined||body.address!==undefined)||body.currency==='USDT'&&(body.bankId!==undefined||body.bankDetails!==undefined))fail();
  ...
  if(body.currency==='INR'){
   if(body.bankDetails!==undefined){
    if(body.bankId!==undefined)fail();
    v.exactFields(body.bankDetails,['bankName','beneficiaryName','accountNumber','ifsc']);
    const d=body.bankDetails;if(typeof d.accountNumber!=='string'||! /^[0-9]{6,24}$/.test(d.accountNumber)||typeof d.ifsc!=='string'||! /^[A-Z]{4}0[A-Z0-9]{6}$/.test(d.ifsc))fail();
    destination={bankName:v.text(d.bankName),beneficiaryName:v.text(d.beneficiaryName),accountNumber:d.accountNumber,ifsc:d.ifsc,source:'request_details'};
   }else{const bank=await banks.owned(client,userId,body.bankId);destination={bankId:bank.id,bankVersion:bank.version,beneficiaryName:bank.details.holderName,accountNumber:bank.details.accountNumber,ifsc:bank.details.ifsc};}
   snapshot.withdrawal={...inrWithdrawal(amount),policy:POLICY.version,feePercent:'0.5'};
  }
  else{if(body.network!==snapshot.settings.depositNetwork)fail();address(body.network,body.address);destination={network:body.network,address:body.address};const scaled=BigInt(money.fromDecimal(snapshot.settings.inrPerUsdt,'USDT'));if(scaled<=0n)fail('UNAVAILABLE');amount=((BigInt(body.amountMinor)*scaled+9999999999n)/10000000000n).toString();money.minor(amount);snapshot.withdrawal={policy:POLICY.version,feeMinor:'0',netMinor:body.amountMinor,rounding:'INR debit rounded up to one paise'};}
 ...
 async transition(client,r,actor,body,user=false){
  ...
  if(['approve','process'].includes(action)&&r.currency==='INR'){
   const d=JSON.parse(this.crypto.open(r.encrypted_destination,binding('destination',r.id)));
   if(d.source!=='request_details'){const b=await banks.owned(client,r.user_id,d.bankId);if(b.version!==d.bankVersion)fail('CONFLICT');}
  }
  if(['complete','reject','cancel'].includes(action)){
   ...
   await a.journal(client,r.id,action==='complete'?'commission_complete':'commission_release',actor,entries,r.snapshot,{reason:body.reason,feeMinor:r.snapshot.withdrawal?.feeMinor||'0',netMinor:r.snapshot.withdrawal?.netMinor||r.amount_minor,...(completionDigest?{completionDigest}:{})});
  }
