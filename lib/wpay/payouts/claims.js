...
async function queue(client,userId){
 ...
 WHERE p.state='open' AND NOT EXISTS(SELECT 1 FROM wpay_auth.late_payment_reviews l WHERE l.kind='payout' AND l.resource_id=p.id AND l.held_minor>0 AND NOT EXISTS(SELECT 1 FROM wpay_auth.late_payment_resolutions z WHERE z.review_id=l.id)) AND m.tenant_id=$2 AND (p.deadline_at IS NULL OR p.deadline_at>=CURRENT_TIMESTAMP+interval '15 minutes')
...
async function claim(core,client,userId,body){
 ...
 if(await require('../reviews/late-payment').held(client,'payout',p.id)>0n)fail('CONFLICT');
