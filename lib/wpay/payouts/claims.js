...
async function queue(client,userId){
 await expire(client);const options=await eligible(client,userId);if(!options.length)return {orders:[],banks:[],leaseSeconds:LEASE_SECONDS};
 const max=options.reduce((n,b)=>BigInt(b.maxMinor)>n?BigInt(b.maxMinor):n,0n);
 const rows=(await client.query(`SELECT p.id,p.amount_minor::text,p.created_at FROM wpay_auth.payout_orders p JOIN wpay_auth.accounts m ON m.id=p.merchant_id JOIN wpay_auth.eligibility e ON e.account_id=m.id JOIN wpay_auth.account_security s ON s.account_id=m.id WHERE p.state='open' AND p.amount_minor<=$1
 ...
 AND m.status='active' AND e.approval_status='approved' AND s.enabled AND s.factor_version>0 ORDER BY p.created_at,p.id LIMIT 100`,[max.toString()])).rows;
 return {orders:rows.map(p=>({id:p.id,amountMinor:p.amount_minor,currency:'INR',createdAt:p.created_at})),banks:options,leaseSeconds:LEASE_SECONDS,capacityPolicy:'unreserved_consumed_capacity'};
}
async function claim(core,client,userId,body){
 v.id(body.id);v.id(body.bankId);await expire(client,body.id);
 const p=(await client.query('SELECT * FROM wpay_auth.payout_orders WHERE id=$1 FOR UPDATE',[body.id])).rows[0];if(!p)fail('FORBIDDEN');
 ...
 if(p.state!=='open')fail('CONFLICT');
 const m=(await client.query("SELECT a.id FROM wpay_auth.accounts a JOIN wpay_auth.eligibility e ON e.account_id=a.id JOIN wpay_auth.account_security s ON s.account_id=a.id WHERE a.id=$1 AND a.status='active' AND e.approval_status='approved' AND s.enabled AND s.factor_version>0",[p.merchant_id])).rows[0];if(!m)fail('FORBIDDEN');
...
async function settle(core,client,p,actor,reason,admin){
 v.reason(reason);await ledger.lock(client);p=(await client.query('SELECT * FROM wpay_auth.payout_orders WHERE id=$1 FOR UPDATE',[p.id])).rows[0];if(p.state==='successful')return core.project(p);
