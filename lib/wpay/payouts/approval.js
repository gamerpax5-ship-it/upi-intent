...
async function expire(core,client){
 ...
 const rows=(await client.query("SELECT * FROM wpay_auth.payout_orders WHERE state IN('pending_admin','open') AND NOT EXISTS(SELECT 1 FROM wpay_auth.late_payment_reviews l WHERE l.kind='payout' AND l.resource_id=wpay_auth.payout_orders.id AND l.held_minor>0 AND NOT EXISTS(SELECT 1 FROM wpay_auth.late_payment_resolutions z WHERE z.review_id=l.id)) AND deadline_at<CURRENT_TIMESTAMP+interval '15 minutes' ORDER BY deadline_at,id LIMIT 200 FOR UPDATE")).rows;
