...
const GET=['payout/summary','payout/queue','payout/orders','payout/jobs','payout/commission','payout/withdrawals','payout/holds','payout/capabilities','payout/merchant-usdt'];
const POST=['payout/create','payout/bulk','payout/get','payout/search','payout/claim','payout/release','payout/submit','payout/review','payout/resolve','payout/proof','payout/withdrawal/create','payout/withdrawal/get','payout/withdrawal/search','payout/withdrawal/transition','payout/hold/manage','payout/hold/search','payout/capability','payout/commission/search','payout/merchant-usdt/create'];
...
function navigation(groups,context){
 const type=context.principal.type,items=type==='merchant'?[['orders','merchant.payout.view','INR Payouts'],['merchant-usdt','merchant.settlement.view','USDT Settlement']]:type==='user'?[['jobs','user.payout.view','INR Payout Jobs'],['commission','user.commission.view','Commission'],['withdrawals','user.commission_withdrawal.view','Commission Withdrawals']]:[['orders','payout_operations.view','Payout Review'],['withdrawals','commission_withdrawal.view','Commission Withdrawals'],['holds','commission_hold.view','Commission Holds'],['capabilities','payout_operations.view','Payout Bank Capabilities']];
 const children=items.filter(([id,permissionId])=>canUsePermission({...context,permissionId}).allowed&&(type!=='user'||id==='commission'||context.eligibility.initialDepositSatisfied===true)).map(([id,permissionId,label])=>({id:'payout.page.'+id,destinationId:'payout.'+id,permissionId,label,routeStatus:'implemented',descriptorOnly:false,blockers:[]}));
 if(children.length)groups.push({id:'payout.group.operations',label:'Payouts & Commission',children});return groups;
}
function preparation(context,row,operation){const user=operation==='payout/submit';if(row.account_type!==(user?'user':'merchant'))fail('FORBIDDEN');permit(context,user?'user.payout.submit':'merchant.payout.create',resource(row,user?'record':'create'));recent(row);}
...
async function run(core,client,row,context,operation,body,prepared){
 ...
 if(['payout/create','payout/bulk'].includes(operation)){preparation(context,row,operation);return operation==='payout/create'?core.create(client,row.id,body):core.bulk(client,row.id,body,prepared);}
 if(operation.startsWith('payout/merchant-usdt')){
  if(!merchant)fail('FORBIDDEN');permit(context,operation.endsWith('/create')?'merchant.settlement.create':'merchant.settlement.view',resource(await target(client,row.id),operation.endsWith('/create')?'create':'list'));
  if(operation.endsWith('/create')){fields(body,['currency','amountMinor','network','address','idempotencyKey']);fail('MERCHANT_FX_UNCONFIGURED');}
  return {enabled:false,status:'rate_not_configured',message:'USDT settlement rate not configured',states:['rate_not_configured','requested','review','approved','processing','completed','rejected','cancelled'],creationDisabled:true};
 }
 if(operation==='payout/summary'){
  if(merchant){permit(context,'merchant.payout.view');const b=await ledger.summary(client,row.id);return {available:b.merchantAvailable,reserved:b.merchantPayoutReserved,principal:b.merchantPayoutPrincipal,fees:b.payoutFees,held:b.merchantHeld,currency:'INR',providerConnected:false};}
 ...
 if(['payout/claim','payout/release'].includes(operation)){
  ...
  if(operation==='payout/claim'){v.exactFields(body,['id','bankId']);return core.claim(client,row.id,body);}
 ...
 if(['payout/get','payout/proof','payout/submit','payout/review','payout/resolve'].includes(operation)){
  ...
  if(operation==='payout/get'){v.exactFields(body,['id']);const detail=await core.detail(client,p,merchant?'merchant':user?'user':'admin');if(admin&&detail.proof)detail.proof.downloadAllowed=canUsePermission({...context,permissionId:'payout_operations.proof'}).allowed;return detail;}
  if(operation==='payout/proof'){if(merchant)fail('FORBIDDEN');if(admin){await payoutScope(client,row,context,body.id,'payout_operations.proof');recent(row);}v.exactFields(body,['id','proofId']);return core.download(client,p,body.proofId,row.id);}
  recent(row);
 ...
 if(['payout/orders','payout/jobs','payout/search'].includes(operation)){
  ...
  const rows=(await client.query(`SELECT p.*,c.created_at AS claimed_at,c.expires_at AS claim_expires_at,s.created_at AS submitted_at FROM wpay_auth.payout_orders p JOIN wpay_auth.accounts m ON m.id=p.merchant_id LEFT JOIN LATERAL(SELECT * FROM wpay_auth.payout_claims c WHERE c.payout_id=p.id ORDER BY c.created_at DESC,c.id LIMIT 1)c ON true LEFT JOIN wpay_auth.payout_submissions s ON s.payout_id=p.id
   ...
   AND ($3::text[] IS NULL OR (m.tenant_id=ANY($3) AND NOT EXISTS(SELECT 1 FROM wpay_auth.payout_claims j JOIN wpay_auth.accounts u ON u.id=j.user_id WHERE j.payout_id=p.id AND NOT(u.tenant_id=ANY($3))))) AND ($4::text IS NULL OR p.state=$4) AND ($5::text IS NULL OR p.reference=$5)
  ...
  return {orders:rows.slice(0,filter.limit).map(r=>{const out=core.project(r);if(user){delete out.percentageFeeMinor;delete out.fixedFeeMinor;delete out.fixedFeeCurrency;delete out.reserveMinor;}return out;}),hasMore:rows.length>filter.limit,offset:filter.offset};
 }
