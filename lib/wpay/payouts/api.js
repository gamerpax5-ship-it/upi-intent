...
async function run(core,client,row,context,operation,body,prepared){
 ...
 if(operation==='payout/summary'){
  if(merchant){
   permit(context,'merchant.payout.view');const b=await ledger.summary(client,row.id),states=(await client.query('SELECT state,count(*)::integer AS n FROM wpay_auth.payout_orders WHERE merchant_id=$1 GROUP BY state',[row.id])).rows,counts=Object.fromEntries(states.map(x=>[x.state,x.n]));
