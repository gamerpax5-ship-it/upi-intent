...
const GET=['payout/summary','payout/queue','payout/orders','payout/jobs','payout/commission','payout/withdrawals','payout/holds','payout/capabilities','payout/merchant-usdt','payout/merchant-usdt-admin'];
...
async function run(core,client,row,context,operation,body,prepared){
 ...
 if(operation.startsWith('payout/merchant-usdt')){
  if(merchant){
   ...
   const summary=await core.merchantSettlements.summary(client,row.id),history=await core.merchantSettlements.list(client,row.id,{offset:0,limit:50});
