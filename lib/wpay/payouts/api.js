...
const POST=['payout/late/open','payout/late/search','payout/late/proof','payout/late/decide','payout/dispute/open','payout/dispute/respond','payout/dispute/resolve','payout/dispute/search','payout/approval/search','payout/approval/decide','payout/template','payout/merchant-usdt/search','payout/merchant-usdt/get','payout/create','payout/bulk','payout/bulk/validate','payout/get','payout/search','payout/claim','payout/release','payout/submit','payout/review','payout/resolve','payout/proof','payout/withdrawal/create','payout/withdrawal/get','payout/withdrawal/search','payout/withdrawal/transition','payout/hold/manage','payout/hold/search','payout/capability','payout/commission/search','payout/merchant-usdt/create','payout/merchant-usdt/transition'];
...
function preparation(context,row,operation){const user=['payout/submit','payout/dispute/respond','payout/late/open'].includes(operation);if(row.account_type!==(user?'user':'merchant'))fail('FORBIDDEN');permit(context,user?'user.payout.submit':operation==='payout/dispute/open'?'merchant.payout.review':'merchant.payout.create',resource(row,user||operation==='payout/dispute/open'?'record':'create'));recent(row);}
...
async function run(core,client,row,context,operation,body,prepared){
 if(operation.startsWith('payout/late/'))return require('../reviews/late-api').run(core,client,row,context,'payout',operation,body,prepared);
 ...
 if(operation.startsWith('payout/dispute/')){
  ...
  if(operation==='payout/dispute/search'){
   ...
   const rows=(await client.query(`SELECT p.id,p.reference,p.amount_minor::text,d.merchant_id,d.user_id,m.name AS merchant_name,u.name AS user_name,
    d.commission_minor::text,d.reason,d.statement_id,d.coverage_from,d.coverage_through,d.created_at,
    (SELECT pr.proof_id FROM wpay_auth.payout_dispute_responses pr WHERE pr.dispute_id=d.id AND pr.proof_id IS NOT NULL ORDER BY pr.created_at DESC,pr.id DESC LIMIT 1) AS response_proof_id,
    COALESCE(r.outcome,'pending') AS status
    FROM wpay_auth.payout_disputes d JOIN wpay_auth.payout_orders p ON p.id=d.payout_id
    JOIN wpay_auth.accounts m ON m.id=d.merchant_id JOIN wpay_auth.accounts u ON u.id=d.user_id
    LEFT JOIN wpay_auth.payout_dispute_resolutions r ON r.dispute_id=d.id
    ...
    AND ($4::text IS NULL OR COALESCE(r.outcome,'pending')=$4)
    ORDER BY d.created_at DESC,d.id LIMIT $5 OFFSET $6`,[merchant?row.id:null,user?row.id:null,admin?scope.tenantIds:null,f.state??null,f.limit+1,f.offset])).rows;
   return {orders:rows.slice(0,f.limit).map(r=>({id:r.id,reference:r.reference,amountMinor:r.amount_minor,merchantId:r.merchant_id,merchantName:r.merchant_name,userId:r.user_id,userName:r.user_name,commissionMinor:r.commission_minor,reason:r.reason,statementId:r.statement_id,responseProofId:r.response_proof_id,coverageFrom:r.coverage_from,coverageThrough:r.coverage_through,createdAt:r.created_at,status:r.status})),hasMore:rows.length>f.limit,offset:f.offset};
  }
 ...
 if(operation.startsWith('payout/merchant-usdt')){
  ...
  if(operation==='payout/merchant-usdt-admin'){
   ...
   return {requests:rows.map(r=>{const out=core.merchantSettlements.project(r,true),d=out.destination;delete out.destination;return {...out,merchantId:r.merchant_id,merchantName:r.merchant_name,tenantId:r.tenant_id,destinationSummary:(d.network||'USDT')+' · '+String(d.address||'').slice(0,6)+'…'+String(d.address||'').slice(-4)};})};
  }
 ...
 if(['payout/orders','payout/jobs','payout/search'].includes(operation)){
  ...
  const rows=(await client.query(`SELECT p.*,EXISTS(SELECT 1 FROM wpay_auth.payout_claims lc WHERE lc.payout_id=p.id AND lc.user_id=$2 AND lc.state='expired' AND NOT EXISTS(SELECT 1 FROM wpay_auth.late_payment_reviews lr WHERE lr.kind='payout' AND lr.claim_id=lc.id)) AS can_late,dr.outcome AS dispute_outcome,c.user_id AS claim_user_id,u.name AS claim_user_name,c.created_at AS claimed_at,c.expires_at AS claim_expires_at,s.id AS submission_id,s.created_at AS submitted_at,
   CASE WHEN s.encrypted_evidence IS NOT NULL THEN s.encrypted_evidence ELSE NULL END AS encrypted_submission_evidence FROM wpay_auth.payout_orders p JOIN wpay_auth.accounts m ON m.id=p.merchant_id LEFT JOIN LATERAL(SELECT * FROM wpay_auth.payout_claims c WHERE c.payout_id=p.id ORDER BY c.created_at DESC,c.id LIMIT 1)c ON true LEFT JOIN wpay_auth.accounts u ON u.id=c.user_id LEFT JOIN wpay_auth.payout_submissions s ON s.payout_id=p.id LEFT JOIN wpay_auth.payout_disputes pd ON pd.payout_id=p.id LEFT JOIN wpay_auth.payout_dispute_resolutions dr ON dr.dispute_id=pd.id
  ...
  return {orders:rows.slice(0,filter.limit).map(r=>{const out=core.project(r);if(r.claim_user_id){out.claimUserId=r.claim_user_id;out.claimUserName=r.claim_user_name||null;}if(r.encrypted_submission_evidence){try{out.utr=JSON.parse(core.crypto.open(r.encrypted_submission_evidence,require('./core').binding('evidence',r.submission_id))).utr||null;}catch{out.utr=null;}}if(r.dispute_outcome==='payment_invalid')out.status='reversed';if(user){out.canLateProof=r.can_late;delete out.percentageFeeMinor;delete out.fixedFeeMinor;delete out.fixedFeeCurrency;delete out.reserveMinor;}return out;}),hasMore:rows.length>filter.limit,offset:filter.offset};
 }
 ...
 if(['payout/withdrawals','payout/withdrawal/search'].includes(operation)){
  ...
  return {withdrawals:rows.slice(0,f.limit).map(r=>{const out=core.withdrawals.project(r),result={...out,...(admin?{userId:r.user_id}:{})};if(admin){const d=JSON.parse(core.withdrawals.crypto.open(r.encrypted_destination,'wpay-withdrawal-destination:'+r.id));result.destinationSummary=r.currency==='INR'?(d.bankName||'Bank')+' · ••••'+String(d.accountNumber||'').slice(-4)+' · '+(d.ifsc||''):(d.network||'USDT')+' · '+String(d.address||'').slice(0,6)+'…'+String(d.address||'').slice(-4);}return result;}),hasMore:rows.length>f.limit,offset:f.offset};
 }
