...
try{
 ...
 if(range.e.r>100||range.e.c>7||range.s.r!==0||range.s.c!==0)throw new Error();
 ...
 const header=rows.shift(),headers=JSON.stringify(header)===JSON.stringify(expanded)?expanded:JSON.stringify(header)===JSON.stringify(legacy)?legacy:null;
