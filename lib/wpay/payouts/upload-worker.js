...
try{
 ...
 if(range.e.r>100||range.e.c>8||range.s.r!==0||range.s.c!==0)throw new Error();
 for(const [key,cell] of Object.entries(sheet))if(!key.startsWith('!')&&(cell.f||cell.l))throw new Error();
 const rows=XLSX.utils.sheet_to_json(sheet,{header:1,raw:true,defval:'',blankrows:true}),legacy=['reference','beneficiaryName','accountNumber','ifsc','amountINR','note'],expanded=['reference','beneficiaryName','bankName','accountNumber','ifsc','upiId','amountINR','note'];
 const bank=['reference','beneficiaryName','bankName','accountNumber','ifsc','amountINR','durationMinutes','note'],upi=['reference','beneficiaryName','upiId','amountINR','durationMinutes','note'];
 const header=rows.shift(),headers=JSON.stringify(header)===JSON.stringify(bank)?bank:JSON.stringify(header)===JSON.stringify(upi)?upi:JSON.stringify(header)===JSON.stringify(expanded)?expanded:JSON.stringify(header)===JSON.stringify(legacy)?legacy:null;
 if(!headers||!rows.length||rows.length>100)throw new Error();
