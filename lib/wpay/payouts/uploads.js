...
function template(){
 ...
 const instructions=XLSX.utils.aoa_to_sheet([
  ['WPay Merchant Bulk Payout Template'],
  ['Rule','Requirement'],
  ['1','Do not rename or reorder columns on Bulk Payouts.'],
  ['2','reference, beneficiaryName, bankName, accountNumber, ifsc, amountINR are required.'],
  ['3','upiId and note are optional.'],
  ['4','Account number must remain text; amountINR must be positive with max two decimals.'],
  ['5','The complete batch is atomic and cannot exceed current Merchant available balance including fees.'],
  ['6','Merchant references must be unique inside the batch and against existing payouts.'],
  ['7','Maximum 100 payout rows and 1 MiB upload size.']
 ]);
 instructions['!cols']=[{wch:10},{wch:95}];
 XLSX.utils.book_append_sheet(book,sheet,'Bulk Payouts');XLSX.utils.book_append_sheet(book,instructions,'Instructions');
 const bytes=XLSX.write(book,{type:'buffer',bookType:'xlsx',compression:true});
 if(!Buffer.isBuffer(bytes)||bytes.length<4||bytes.subarray(0,2).toString()!=='PK')fail('UNAVAILABLE');
 return {name:'WPay-Merchant-Bulk-Payout-Template.xlsx',contentType:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',data:bytes.toString('base64')};
}
