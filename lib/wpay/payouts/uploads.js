...
function template(mode){
 if(mode!==undefined&&! ['bank','upi'].includes(mode))fail();
 const XLSX=require('../../../public/vendor/smart-upi-parser-runtime/xlsx.full.min.js');
 const headers=mode==='upi'?['reference','beneficiaryName','upiId','amountINR','durationMinutes','note']:mode==='bank'?['reference','beneficiaryName','bankName','accountNumber','ifsc','amountINR','durationMinutes','note']:['reference','beneficiaryName','bankName','accountNumber','ifsc','upiId','amountINR','note'];
 ...
 return {name:mode?'WPay-'+mode+'-Payout-Template.xlsx':'WPay-Merchant-Bulk-Payout-Template.xlsx',contentType:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',data};
}
