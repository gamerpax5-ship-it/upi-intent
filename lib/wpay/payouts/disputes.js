...
async function holds(c,{ids,tenants}){
 return (await c.query(`SELECT d.id,d.user_id AS owner_id,d.amount_minor::text,'INR' AS currency,CASE WHEN r.dispute_id IS NULL THEN 'active' ELSE 'released' END AS state,'payout_dispute_capacity' AS category,d.reason,p.reference,d.created_at,r.created_at AS released_at FROM wpay_auth.payout_disputes d JOIN wpay_auth.payout_orders p ON p.id=d.payout_id JOIN wpay_auth.accounts a ON a.id=d.user_id LEFT JOIN wpay_auth.payout_dispute_resolutions r ON r.dispute_id=d.id WHERE ($1::uuid[] IS NULL OR d.user_id=ANY($1)) AND ($2::text[] IS NULL OR a.tenant_id=ANY($2)) ORDER BY d.created_at DESC,d.id LIMIT 100`,[ids,tenants])).rows;
}
module.exports={open,respond,resolve,detail,coverage,withinWindow,holds};
