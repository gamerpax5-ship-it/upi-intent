...
class MerchantSettlements{
 ...
 async summary(client,merchantId){
  ...
  return {enabled:true,currency:'INR',available:balance.merchantAvailable,reserved:balance.merchantSettlementReserved,principal:balance.merchantSettlementPrincipal,
   ...
   maxUsdtMinor:this.quote(balance.merchantAvailable,terms.rateMinor)};
 }
 ...
 async request(client,merchantId,body){
  fields(body,['idempotencyKey','amountMinor','network','address']);v.reference(body.idempotencyKey);money.minor(body.amountMinor);
  ...
  const balance=await ledger.summary(client,merchantId),amount=money.minor(body.amountMinor);if(amount<=0n||BigInt(balance.merchantAvailable)<amount)fail('INSUFFICIENT_BALANCE');
  const usdtMinor=this.quote(body.amountMinor,terms.rateMinor),id=randomUUID(),snapshot={id:terms.id,version:terms.version,rate:terms.rate,network:body.network,rounding:'floor-to-six-decimals'};
  ...
  const row=(await client.query(`INSERT INTO wpay_auth.merchant_settlement_withdrawals(id,merchant_id,idempotency_key,payload_digest,inr_minor,usdt_minor,snapshot,encrypted_destination)
   VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,[id,merchantId,body.idempotencyKey,fingerprint,body.amountMinor,usdtMinor,snapshot,this.crypto.seal(JSON.stringify(destination),binding('destination',id))])).rows[0];
  await accounting.journal(client,id,'merchant_settlement_reserve',merchantId,ledger.pair(merchantId,'merchant_settlement_reserved',body.amountMinor),snapshot);
