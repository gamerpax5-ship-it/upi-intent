...
function order(body){
 fields(body,['reference','idempotencyKey','beneficiaryName','bankName','accountNumber','ifsc','upiId','amountMinor','note']);
 ...
 const bankName=body.bankName===undefined?'':v.text(body.bankName,120),upiId=body.upiId===undefined||body.upiId===''?'':String(body.upiId).toLowerCase();
 if(upiId&&!/^[A-Za-z0-9._-]{2,100}@[A-Za-z0-9.-]{2,40}$/.test(upiId))fail();
 return {reference:body.reference,idempotencyKey:body.idempotencyKey,beneficiaryName:v.text(body.beneficiaryName,120),bankName,accountNumber:body.accountNumber,ifsc:body.ifsc,upiId,amountMinor:body.amountMinor,note:text(body.note??'',300)};
}
function page(body={}){
