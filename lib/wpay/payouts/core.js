...
class Payouts{
 constructor({gateway,crypto,scanner=null}){this.gateway=gateway;this.crypto=crypto;this.scanner=scanner;this.withdrawals=new Withdrawals(crypto);}
 ...
 async create(client,merchantId,body,batchId=null){
  ...
  const beneficiary={beneficiaryName:value.beneficiaryName,accountNumber:value.accountNumber,ifsc:value.ifsc,note:value.note};
 ...
 async detail(client,row,role){
  ...
  if(role!=='queue'){
   const beneficiary=JSON.parse(this.crypto.open(row.encrypted_beneficiary,binding('beneficiary',row.id)));out.beneficiary={beneficiaryName:beneficiary.beneficiaryName,accountNumber:beneficiary.accountNumber,ifsc:beneficiary.ifsc};if(role==='merchant')out.note=beneficiary.note;
