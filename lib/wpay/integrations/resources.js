...
class Resources {
  ...
  async run(client,row,operation,body){
    ...
    if(operation==='resources'){
      const result=await client.query("SELECT id,resource_kind AS kind,resource_id AS reference,status,consent_at,verified_at,valid_until FROM wpay_auth.resource_links WHERE account_id=$1 AND status<>'revoked' ORDER BY consent_at DESC,id LIMIT 100",[row.id]);
      const sourceConnected=this.reader ? await this.reader.ready() : false;
      return {sourceConnected,message:sourceConnected?'Ownership verification required':'Device/source not connected',links:result.rows,revealAvailable:false,
    ...
    const views={device:['device','otp','transactions'],statement_import:['statement'],order:['order'],payment_link:['order']};
    if(!views[link.resource_kind]?.includes(body.view))throw new AuthError('FORBIDDEN');
    if(['transactions','statement'].includes(body.view)){
    ...
    const result=await this.reader.read(link,body.view,before);
    ...
    return {...result,financiallyAccounted:false,settled:false,ownershipVerified:true,evidenceVerified:false};
  }
}
