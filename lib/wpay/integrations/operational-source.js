...
function config(env){
 ...
 try{const u=new URL(env.WPAY_OPERATIONAL_READER_DATABASE_URL),port=Number(u.port||5432),db=u.pathname.slice(1),local=['127.0.0.1','[::1]'].includes(u.hostname),privateHost=/^[a-z0-9][a-z0-9-]*\.railway\.internal$/.test(u.hostname)&&env.WPAY_HOSTED_NETWORK==='railway-private-network';
  if(!['postgresql:','postgres:'].includes(u.protocol)||decodeURIComponent(u.username)!=='wpay_operational_reader'||!u.password||u.search||u.hash||!/^\w{1,63}$/.test(db)||(!local&&!privateHost)||!Number.isInteger(port)||port<1||port>65535||!local&&port!==5432||env.WPAY_OPERATIONAL_READER_TARGET!==`${u.hostname}:${port}/${db}`)throw 0;
  return {host:u.hostname.replace(/^\[|\]$/g,''),port,database:db,user:'wpay_operational_reader',password:decodeURIComponent(u.password),ssl:false,max:2,connectionTimeoutMillis:3000,idleTimeoutMillis:10000,statement_timeout:3000,options:'-c search_path=pg_catalog -c default_transaction_read_only=on',application_name:'wpay-operational-source'};
 }catch{unavailable();}
}
