...
async function loadCheckout(){
 ...
 return async function checkout(req,res,{service,origin,secure,readBody,send}){
  ...
  const approved=result.status==='successful'&&result.evidenceStatus==='admin_approved';
  if(!api[2]){
  ...
  }else send(res,200,{status:(verified||approved)?'success':['verification_pending','recovery_review'].includes(result.status)?'pending':result.status,verified,approved,amountMinor:result.amountMinor,currency:result.currency,reference:result.reference,orderId:result.id,paidAt:result.paidAt,expiresAt:result.expiresAt,bankVerified:verified,approvalMethod:approved?'admin_manual':verified?'bank_evidence':null,message:approved?'Payment approved by administrator':verified?'Payment independently verified':'Waiting for independent payment evidence'});
  return true;
 };
}
