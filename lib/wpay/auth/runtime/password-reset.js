...
function valid(row){if(!pending(row)||row.account_type!=='employee'||!row.temporary_expires_at||+row.temporary_expires_at<=+row.database_now)fail();}
...
async function complete(service,rawToken,body,entryRole){
 ...
 return transaction(service.repository.pool,async client=>{
  ...
  // No application session is created here. Existing mandatory MFA follows reset.
