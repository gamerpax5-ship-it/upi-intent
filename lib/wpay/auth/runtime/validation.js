...
function password(value) {
  if (typeof value !== "string" || !value.isWellFormed() || [...value].length < 15 ||
...
function registration(value) {
  ...
  return { name: name(value.name), email: email(value.email), password: password(value.password), accountType: value.accountType };
}
function login(value) {
  exactFields(value, ["email", "password"]);
  return { email: email(value.email), password: password(value.password) };
}
module.exports = { exactFields, password, email, name, registration, login };
