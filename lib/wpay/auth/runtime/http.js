...
const { GET: PARKING_GET, POST: PARKING_POST } = require("../../parking/api");
...
const ASSETS = Object.freeze({
  [PREFIX+"user-burgundy.css"]:["user-burgundy.css","text/css"],
  [PREFIX+"user-burgundy-live.css"]:["user-burgundy-live.css","text/css"],
  [PREFIX+"admin-utr.js"]:["admin-utr.js","text/javascript"],
  [PREFIX+"checkout-status.js"]:["checkout-status.js","text/javascript"],
  [PREFIX+"merchant-premium.css"]:["merchant-premium.css","text/css"],
  [PREFIX+"merchant-premium-icons.js"]:["merchant-premium-icons.js","text/javascript"],
  [PREFIX+"merchant-premium.js"]:["merchant-premium.js","text/javascript"],
  [PREFIX+"reference-layouts.js"]: ["reference-layouts.js","text/javascript"],
  [PREFIX+"reference-presentation.js"]: ["reference-presentation.js","text/javascript"],
  [PREFIX+"user-logo.svg"]: ["user-logo.svg","image/svg+xml"],
  [PREFIX+"user-burgundy-banks.js"]: ["user-burgundy-banks.js","text/javascript"],
  [PREFIX+"user-burgundy-dashboard.js"]: ["user-burgundy-dashboard.js","text/javascript"],
  [PREFIX+"reference-dashboard.js"]: ["reference-dashboard.js","text/javascript"],

  [PREFIX+"reference-history.js"]: ["reference-history.js","text/javascript"],
  [PREFIX+"user-live.html"]: ["user-live.html","text/html"],
  [PREFIX+"merchant-live.html"]: ["merchant.html","text/html"],
  [PREFIX+"user-icons.js"]: ["user-icons.js","text/javascript"],
  [PREFIX+"merchant-icons.js"]: ["merchant-icons.js","text/javascript"],
  [PREFIX+"reference-ui.js"]: ["reference-ui.js","text/javascript"],
  [PREFIX+"reference-navigation.js"]: ["reference-navigation.js","text/javascript"],
  [PREFIX+"reference-live.css"]: ["reference-live.css","text/css"],

  [PREFIX+"prototype-xlsx.js"]: ["../../../public/vendor/smart-upi-parser-runtime/xlsx.full.min.js","text/javascript"],
  [PREFIX+"user-prototype.css"]: ["user-prototype.css","text/css"],
  [PREFIX+"merchant-prototype.css"]: ["merchant-prototype.css","text/css"],
 [PREFIX+"admin.html"]: ["admin.html","text/html"], [PREFIX+"admin-ui.css"]: ["admin-ui.css","text/css"], [PREFIX+"admin-ui.js"]: ["admin-ui.js","text/javascript"], [PREFIX+"admin-pages.js"]: ["admin-pages.js","text/javascript"], [PREFIX+"admin-finance.js"]: ["admin-finance.js","text/javascript"], [PREFIX+"admin-upi.js"]: ["admin-upi.js","text/javascript"], [PREFIX+"admin-prototype.css"]: ["admin-prototype.css","text/css"],
 [PREFIX]: ["index.html", "text/html"], [PREFIX+"user.html"]: ["user.html","text/html"], [PREFIX+"merchant.html"]: ["merchant.html","text/html"],
  ...
  [PREFIX + "device-setup.js"]: ["device-setup.js", "text/javascript"],
  [PREFIX + "late-review.js"]: ["late-review.js", "text/javascript"],
  [PREFIX + "user-burgundy-pages.js"]: ["user-burgundy-pages.js", "text/javascript"],
  ...
  [PREFIX + "business-locales.js"]: ["business-locales.js", "text/javascript"], [PREFIX+"parking.js"]: ["parking.js","text/javascript"], [PREFIX+"role-dashboard.js"]: ["role-dashboard.js","text/javascript"], [PREFIX+"user-role.css"]: ["user-role.css","text/css"], [PREFIX+"merchant-role.css"]: ["merchant-role.css","text/css"] });
...
async function startAuthServer({ service, port = 4174, hostedOrigin, readiness }) {
  ...
  const assets = new Map(), encodedAssets = new Map();
  ...
  for (const [url, [file, type]] of Object.entries(ASSETS)) {
    ...
    assets.set(url, [bytes, type]); encodedAssets.set(url,staticResponse.asset(bytes,type));
  }
  const server = http.createServer({ maxHeaderSize: 16384 }, async (req, res) => {
    ...
    try {
      ...
      if(entry&&req.method==='GET'){
        const role=entry[1],assetKey=role==='user'?PREFIX+'user-live.html':role==='merchant'?PREFIX+'merchant-live.html':role==='admin'?PREFIX+'admin.html':PREFIX;
        ...
        const rendered=role==='admin'||role==='employee'
          ...
          : bytes;
        if(role==='user'||role==='merchant') res.setHeader("Content-Security-Policy", "default-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'; connect-src 'self'; img-src 'self' data:; base-uri 'none'; frame-ancestors 'none'; form-action 'none'");
        return role==='user'||role==='merchant'?staticResponse.serve(req,res,encodedAssets.get(assetKey)):send(res,200,rendered,type);
      }
      ...
      const authPolicy=require('./transport').roleTransport(policy,entryRole);
      const {session:SESSION_COOKIE,csrf:CSRF_COOKIE,challenge:CHALLENGE_COOKIE}=authPolicy;
      let offset = 0;
      if (url.search) {
      ...
      if (req.method === "GET" && assets.has(url.pathname)) {
        ...
        if (/\/(?:user-live|merchant-live|merchant)\.html$/.test(url.pathname)) res.setHeader("Content-Security-Policy", "default-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'; connect-src 'self'; img-src 'self' data:; base-uri 'none'; frame-ancestors 'none'; form-action 'none'");
        return staticResponse.serve(req,res,encodedAssets.get(url.pathname),type!=="text/html");
      }
      if (req.method === "GET" && route === "favicon.ico") { res.statusCode = 204; return res.end(); }
      const cookies = readCookies(req.headers.cookie, authPolicy);
      const session = cookies[SESSION_COOKIE];
      const challenge = cookies[CHALLENGE_COOKIE];
      if (req.method === "GET") {
        if (!["me", "navigation", "pending-users", "pending-merchants", "security", "approval-options", "apk", "apk/download", "resources", ...BUSINESS_GET, ...FUNDING_GET, ...ONBOARDING_GET, ...GATEWAY_GET, ...PAYOUT_GET, ...PARKING_GET, ...OPERATIONS_GET,...PANEL_GET].includes(route)) throw new AuthError("NOT_FOUND");
      ...
      if (req.headers.origin !== expectedOrigin) throw new AuthError("CSRF_FAILED");
      if (!["csrf", "register", "login", "password/reset", "logout", "logout-all", "refresh", "approval", "locale", "security/admin-email", "security/admin-password", "security/admin-reauth", "security/enable", "security/disable", "security/replace", "security/regenerate", "security/stepup", "security/password", "mfa/setup", "mfa/verify", "mfa/recover", "mfa/complete", "resources/request", "resources/revoke", "resources/read", ...BUSINESS_POST, ...FUNDING_POST, ...ONBOARDING_POST, ...GATEWAY_POST, ...PAYOUT_POST, ...PARKING_POST, ...OPERATIONS_POST,...PANEL_POST].includes(route)) throw new AuthError("NOT_FOUND");
      const body = await readBody(req,['onboarding/upload','operations/statement/upload','payout/bulk','payout/bulk/validate','payout/submit','payout/dispute/open','payout/dispute/respond','parking/submit'].includes(route)?1402200:route.startsWith('panel/support/')?12288:4096);
      if (route === "csrf") {
        exactFields(body, []);
        const result = await service.csrf(req.socket.remoteAddress, session, challenge);
        res.setHeader("Set-Cookie", cookie(CSRF_COOKIE, result.cookie, 900, authPolicy));
      ...
      if (route === "login") {
        return sendAuthResult(res, await service.login(body, req.socket.remoteAddress,entryRole), authPolicy);
      }
      if(route==='password/reset'){
        await service.repository.throttle('login',req.socket.remoteAddress);
        return sendAuthResult(res,await require('./password-reset').complete(service,challenge,body,entryRole),authPolicy);
      }
      if (route.startsWith("mfa/")) return sendAuthResult(res,await service.mfa.challenge(challenge,route.split("/")[1],body,entryRole),authPolicy);
      if (["approval","locale","security/admin-email", "security/admin-password", "security/admin-reauth", "security/admin-email","security/admin-password","security/admin-reauth","security/enable","security/disable","security/replace","security/regenerate","security/stepup","security/password","resources/request","resources/revoke","resources/read",...BUSINESS_POST, ...FUNDING_POST, ...ONBOARDING_POST, ...GATEWAY_POST, ...PAYOUT_POST, ...PARKING_POST, ...OPERATIONS_POST,...PANEL_POST].includes(route)) {
        if (route.startsWith("security/")) await service.repository.throttle("login",req.socket.remoteAddress);
        return sendAuthResult(res,await service.authenticated(session,route,0,body,entryRole),authPolicy);
      }
      exactFields(body, []);
      const result = await service.authenticated(session, route,0,{},entryRole);
      if (route !== "refresh") res.setHeader("Set-Cookie", [cookie(SESSION_COOKIE, "", 0,authPolicy), cookie(CSRF_COOKIE, "", 0,authPolicy),cookie(CHALLENGE_COOKIE,"",0,authPolicy)]);
      return send(res, 200, result);
    } catch (error) {
