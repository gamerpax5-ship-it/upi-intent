...
const ASSETS = Object.freeze({
  ...
  [PREFIX+"merchant-live.html"]: ["merchant-live.html","text/html"],
  ...
  const server = http.createServer({ maxHeaderSize: 16384 }, async (req, res) => {
    ...
    try {
      ...
      if (req.method === "GET" && assets.has(url.pathname)) {
        ...
        if (/\/(user|merchant)-live\.html$/.test(url.pathname)) res.setHeader("Content-Security-Policy", "default-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'; connect-src 'self'; img-src 'self' data:; base-uri 'none'; frame-ancestors 'none'; form-action 'none'");
      ...
      if (["approval","locale","security/admin-email", "security/admin-password", "security/admin-reauth", "security/admin-email","security/admin-password","security/admin-reauth","security/replace","security/regenerate","security/stepup","security/password","resources/request","resources/revoke","resources/read",...BUSINESS_POST, ...FUNDING_POST, ...ONBOARDING_POST, ...GATEWAY_POST, ...PAYOUT_POST, ...PARKING_POST, ...OPERATIONS_POST,...PANEL_POST].includes(route)) {
