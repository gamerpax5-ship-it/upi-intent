  ...
  const server = http.createServer({ maxHeaderSize: 16384 }, async (req, res) => {
    ...
    try {
      ...
      if(entry&&req.method==='GET'){
        const [bytes,type]=assets.get(PREFIX);
        return send(res,200,Buffer.from(bytes.toString('utf8').replace('<head>','<head><meta name="wpay-entry-role" content="'+entry[1]+'">')),type);
      }
      ...
      if (req.method === "GET") {
        if (!["me", "navigation", "pending-users", "pending-merchants", "security", "approval-options", "apk", "apk/download", "resources", ...BUSINESS_GET, ...FUNDING_GET, ...ONBOARDING_GET, ...GATEWAY_GET, ...PAYOUT_GET, ...OPERATIONS_GET].includes(route)) throw new AuthError("NOT_FOUND");
      ...
      if (req.headers.origin !== expectedOrigin) throw new AuthError("CSRF_FAILED");
      if (!["csrf", "register", "login", "logout", "logout-all", "refresh", "approval", "locale", "security/replace", "security/regenerate", "security/stepup", "mfa/setup", "mfa/verify", "mfa/recover", "mfa/complete", "resources/request", "resources/revoke", "resources/read", ...BUSINESS_POST, ...FUNDING_POST, ...ONBOARDING_POST, ...GATEWAY_POST, ...PAYOUT_POST, ...OPERATIONS_POST].includes(route)) throw new AuthError("NOT_FOUND");
      const body = await readBody(req,['onboarding/upload','operations/statement/upload','payout/bulk','payout/submit'].includes(route)?1402200:4096);
      if (route === "csrf") {
        exactFields(body, []);
      ...
      if (["approval","locale","security/replace","security/regenerate","security/stepup","resources/request","resources/revoke","resources/read",...BUSINESS_POST, ...FUNDING_POST, ...ONBOARDING_POST, ...GATEWAY_POST, ...PAYOUT_POST, ...OPERATIONS_POST].includes(route)) {
