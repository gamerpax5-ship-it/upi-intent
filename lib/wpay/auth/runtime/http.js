...
async function startAuthServer({ service, port = 4174, hostedOrigin, readiness }) {
  ...
  const server = http.createServer({ maxHeaderSize: 16384 }, async (req, res) => {
    ...
    try {
      ...
      if (req.method === "GET") {
        if (!["me", "navigation", "pending-users", "pending-merchants", "security", "approval-options", "apk", "apk/download", "resources", ...BUSINESS_GET].includes(route)) throw new AuthError("NOT_FOUND");
      ...
      if (!["csrf", "register", "login", "logout", "logout-all", "refresh", "approval", "locale", "security/replace", "security/regenerate", "security/stepup", "mfa/setup", "mfa/verify", "mfa/recover", "mfa/complete", "resources/request", "resources/revoke", "resources/read", ...BUSINESS_POST].includes(route)) throw new AuthError("NOT_FOUND");
      ...
      if (["approval","locale","security/replace","security/regenerate","security/stepup","resources/request","resources/revoke","resources/read",...BUSINESS_POST].includes(route)) {
