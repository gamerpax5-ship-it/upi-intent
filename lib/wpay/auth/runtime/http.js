...
const {GET:PANEL_GET,POST:PANEL_POST}=require('../../panels/api');
...
async function startAuthServer({ service, port = 4174, hostedOrigin, readiness }) {
  ...
  const server = http.createServer({ maxHeaderSize: 16384 }, async (req, res) => {
    ...
    try {
      ...
      if (req.headers.origin !== expectedOrigin) throw new AuthError("CSRF_FAILED");
      if (!["csrf", "register", "login", "password/reset", "logout", "logout-all", "refresh", "approval", "locale", "security/replace", "security/regenerate", "security/stepup", "mfa/setup", "mfa/verify", "mfa/recover", "mfa/complete", "resources/request", "resources/revoke", "resources/read", ...BUSINESS_POST, ...FUNDING_POST, ...ONBOARDING_POST, ...GATEWAY_POST, ...PAYOUT_POST, ...OPERATIONS_POST,...PANEL_POST].includes(route)) throw new AuthError("NOT_FOUND");
      ...
      if (["approval","locale","security/replace","security/regenerate","security/stepup","resources/request","resources/revoke","resources/read",...BUSINESS_POST, ...FUNDING_POST, ...ONBOARDING_POST, ...GATEWAY_POST, ...PAYOUT_POST, ...OPERATIONS_POST,...PANEL_POST].includes(route)) {
