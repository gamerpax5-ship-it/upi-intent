...
class MfaService {
  ...
  async begin(account) {
    this.ready();
    return this.repository.startChallenge(account.id, account.password_record, (client,row) => this.newChallenge(client,row,row.mfa_enabled ? "login" : "enroll"));
  }
  async challenge(rawToken, operation, body) {
    this.ready();
