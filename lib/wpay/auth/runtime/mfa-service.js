...
const { exactFields, password, passwordInput } = require("./validation");
...
const { requiresIst, validIst } = require("./admin-time-check");
...
class MfaService {
  ...
  async newChallenge(client, row, purpose) {
    ...
    return { challengeToken, stage: purpose === "login" ? "challenge" : "enroll", requiresIst:requiresIst(row) };
  }
  async begin(account,entryRole) {
    this.ready();
    const result = await this.repository.startChallenge(account.id, account.password_record, (client,row) => {requireRole(row,entryRole);const reset=require('./password-reset');return reset.pending(row)?reset.begin(this.repository,client,row):this.newChallenge(client,row,row.mfa_enabled ? "login" : "enroll");});
    return {...result,requiresIst:requiresIst(account)};
  }
  async challenge(rawToken, operation, body, entryRole) {
    ...
    const result = await this.repository.withChallenge(tokenDigest, async (client,row,challenge) => {
      ...
      if(row.temporary_required)throw new AuthError('MFA_FAILED');
      if (operation === "setup") {
        exactFields(body, []);
      ...
      if (operation === "recover") {
        exactFields(body, requiresIst(row) ? ["recoveryCode","istCode"] : ["recoveryCode"]);
        ...
        if(requiresIst(row) && !validIst(body.istCode,row.database_now)) return {failure:"MFA_FAILED"};
      ...
      exactFields(body, requiresIst(row) ? ["code","istCode"] : ["code"]);
  ...
  async fresh(client, row, body, action, sessionDigest) {
    exactFields(body,action === "password" ? ["password","code","newPassword"] : ["password","code"]); passwordInput(body.password); this.ready();
    const current = await this.repository.lockedAccount(client,row.id);
    if(action === "password") {
      if(!["user","merchant"].includes(current.account_type)) throw new AuthError("FORBIDDEN");
      password(body.newPassword,current.account_type);
      if(body.newPassword === body.password) throw new AuthError("INVALID_INPUT");
    }
    ...
    if (action === "password") {
      ...
      await client.query("UPDATE wpay_auth.credentials SET password_record=$2 WHERE account_id=$1",[current.id,record]);
      await client.query("INSERT INTO wpay_auth.panel_audit(id,actor_id,target_id,action) VALUES(gen_random_uuid(),$1,$1,'password_changed')",[current.id]);
