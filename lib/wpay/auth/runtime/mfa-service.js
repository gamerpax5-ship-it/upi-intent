...
class MfaService {
  ...
  async begin(account,entryRole) {
    this.ready();
    return this.repository.startChallenge(account.id, account.password_record, (client,row) => {requireRole(row,entryRole);const reset=require('./password-reset');return reset.pending(row)?reset.begin(this.repository,client,row):this.newChallenge(client,row,row.mfa_enabled ? "login" : "enroll");});
  }
