...
function commercial(type, input, fixedCurrency) {
  ...
  if (type === "merchant") {
    ...
    const ttl=input.paymentLinkTtlSeconds===undefined?300:Number(input.paymentLinkTtlSeconds);
    if(!Number.isInteger(ttl)||ttl<30||ttl>900)throw new AuthError("INVALID_INPUT");
    const rate=input.inrPerUsdt===undefined?null:decimal(input.inrPerUsdt,{positive:true});
