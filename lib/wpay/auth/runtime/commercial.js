...
function commercial(type, input, fixedCurrency) {
  ...
  if (type === "merchant") {
    ...
    const ttl=input.paymentLinkTtlSeconds===undefined||input.paymentLinkTtlSeconds===""?300:Number(input.paymentLinkTtlSeconds);
    if(!Number.isInteger(ttl)||ttl<30||ttl>900)throw new AuthError("INVALID_INPUT");
    const rate=input.inrPerUsdt===undefined||input.inrPerUsdt===""?null:decimal(input.inrPerUsdt,{positive:true});
    return { payinFee: decimal(input.payinFee, { percentage: true }), payoutFee: decimal(input.payoutFee, { percentage: true }),
      fixedPayoutFee: decimal(input.fixedPayoutFee), fixedFeeCurrency: fixedCurrency, paymentLinkTtlSeconds:ttl, inrPerUsdt:rate };
  }
