...
function roleTransport(policy,role){
 if(role===undefined)return policy;
 if(!['admin','merchant','user','employee'].includes(role))throw new AuthError('NOT_FOUND');
 return Object.freeze({...policy,session:policy.session+'_'+role,csrf:policy.csrf+'_'+role,challenge:policy.challenge+'_'+role});
}
module.exports = { transport, DEVELOPMENT, roleTransport };
