...
const DEFAULT_GRANTS = Object.freeze({
  user: Object.freeze(["profile.view", "user.overview.view", "support.view", "guide.view", "user.apk.view", "user.source_events.view", "user.bank_upi.view", "user.bank_upi.submit", "user.bank_upi.update", "user.holds.view", "user.payin_commission.view", ...SELF_SECURITY]),
  ...
  super_admin: Object.freeze(["profile.view", "overview.view", "users.view", "users.approve", "users.reject", "merchants.view", "merchants.approve", "merchants.reject", "apk.view", "bank_upi.view", "bank_upi.review", "bank_upi.approve", "bank_upi.reject", "bank_upi.freeze", "bank_upi.release", "assignments.view", "assignments.update", "routing.view", "ledger.view", "holds.view", "holds.update", ...SELF_SECURITY]),
  ...
  async authenticated(session, operation, offset = 0, body = {}) {
    ...
    for(let attempt=0;;attempt++){
      try { result=await invoke(); break; }
        ...
        if(!operation.startsWith('business/') || !['40001','40P01'].includes(error.code) || attempt>=2)throw error;
      }
    }
