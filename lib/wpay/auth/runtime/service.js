...
const payinDisputeApi = require("../../gateway/payin-dispute-api");
const { PayinDisputes } = require("../../gateway/payin-disputes");
...
const USER_OPERATIONS=['user.live_otp.view','user.device_pairing.view','user.device_pairing.create','user.device_pairing.revoke','user.transaction_history.view','user.payin_dispute.view','user.payin_dispute.respond'];
const ADMIN_OPERATIONS=['devices.pairing.create','devices.revoke','apk_otp_events.view_all','employee_management.view','employee_management.create','employee_management.update','utr_center.view','utr_center.approve','statement_reconciliation.view','statement_reconciliation.upload'];
...
class AuthService {
  constructor(repository, options = {}) {
    ...
    this.payinDisputes = new PayinDisputes({crypto:options.mfaCrypto,scanner:options.uploadScanner});
  ...
  async authenticated(session, operation, offset = 0, body = {}, entryRole) {
    ...
    let prepared;
    if (["payout/bulk","payout/bulk/validate","payout/submit","payout/dispute/open","payout/dispute/respond","payout/late/open"].includes(operation)) {
    ...
    if(["parking/submit","parking/late/open"].includes(operation)){
    ...
    if(["payin-dispute/open","payin-dispute/respond"].includes(operation)){
      await this.repository.withSession(sessionDigest,async (client,row)=>{requireRole(row,entryRole);const context=resolveContext(row);payinDisputeApi.preparation(context,row,operation);});
      prepared=await this.payinDisputes.prepare(operation,body);
    }
    const invoke = () => this.repository.withSession(sessionDigest, async (client, row) => {
      ...
      if (operation.startsWith("payin-dispute/")) return payinDisputeApi.run(this.payinDisputes,client,row,context,operation,body,prepared);
      ...
      if (operation === "approval-options") {
        ...
        const configured=(await client.query('SELECT merchant_inr_per_usdt,merchant_fixed_payout_fee,merchant_payin_fee,merchant_payout_fee,merchant_payment_link_ttl_seconds FROM wpay_auth.admin_commercial_defaults WHERE tenant_id=$1',[row.tenant_id])).rows[0];
        return { fixedFeeCurrency: ["INR","USD","USDT"].includes(this.fixedCurrency) ? this.fixedCurrency : null, depositNetworks: ["TRON-TRC20","ETHEREUM-ERC20"],
          defaultMerchantInrPerUsdt:configured?.merchant_inr_per_usdt||'107',defaultMerchantFixedPayoutFee:configured?.merchant_fixed_payout_fee||'6',
          defaultMerchantPayinFee:configured?.merchant_payin_fee||'1.2',defaultMerchantPayoutFee:configured?.merchant_payout_fee||'0.8',
          defaultMerchantPaymentLinkTtlSeconds:String(configured?.merchant_payment_link_ttl_seconds||300) };
      }
