...
class AuthService {
  ...
  async authenticated(session, operation, offset = 0, body = {}, entryRole) {
    ...
    const invoke = () => this.repository.withSession(sessionDigest, async (client, row) => {
      ...
      if(operation.startsWith('panel/'))return require('../../panels/api').run(client,row,context,operation,body,this.fixedCurrency,this.gateway);
      ...
      if (operation === "navigation") {
        ...
        return { groups: require('../../panels/navigation').navigation(operationNavigation(payoutApi.navigation(gatewayApi.navigation(onboardingNavigation(businessNavigation(groups,context),context),context),context),context),context) };
      }
    ...
    for(let attempt=0;;attempt++){
      try { result=await invoke(); break; }
        ...
        if(!/^(business|funding|onboarding|gateway|payout|operations|panel)\//.test(operation) || !['40001','40P01'].includes(error.code) || attempt>=2)throw error;
      }
    }
