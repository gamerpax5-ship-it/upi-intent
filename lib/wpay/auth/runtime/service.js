...
class AuthService {
  ...
  async authenticated(session, operation, offset = 0, body = {}) {
    ...
    if (["payout/bulk","payout/submit"].includes(operation)) {
      await this.repository.withSession(sessionDigest,async (client,row)=>{const context=resolveContext(row);payoutApi.preparation(context,row,operation);});
    ...
    const invoke = () => this.repository.withSession(sessionDigest, async (client, row) => {
      ...
      if (["resources","resources/request","resources/revoke","resources/read"].includes(operation)) {
        ...
        return this.resources.run(client,row,operation,body);
      }
      ...
      if (operation === "navigation") {
        ...
        return { groups: payoutApi.navigation(gatewayApi.navigation(onboardingNavigation(businessNavigation(groups,context),context),context),context) };
      }
    ...
    for(let attempt=0;;attempt++){
      try { result=await invoke(); break; }
        ...
        if(!/^(business|funding|onboarding|gateway|payout)\//.test(operation) || !['40001','40P01'].includes(error.code) || attempt>=2)throw error;
      }
    }
