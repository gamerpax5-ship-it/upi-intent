  ...
  async authenticated(session, operation, offset = 0, body = {}, entryRole) {
    ...
    const invoke = () => this.repository.withSession(sessionDigest, async (client, row) => {
      ...
      if (operation === "approval-options") {
        ...
        const configured=(await client.query('SELECT merchant_inr_per_usdt FROM wpay_auth.admin_commercial_defaults WHERE tenant_id=$1',[row.tenant_id])).rows[0];
        return { fixedFeeCurrency: ["INR","USD","USDT"].includes(this.fixedCurrency) ? this.fixedCurrency : null, depositNetworks: ["TRON-TRC20","ETHEREUM-ERC20"], defaultMerchantInrPerUsdt:configured?.merchant_inr_per_usdt||'107' };
      }
