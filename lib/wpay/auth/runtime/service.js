...
const DEFAULT_GRANTS = Object.freeze({
  user: Object.freeze([...USER_OPERATIONS,"profile.view", "user.overview.view", "user.deposits.view", "user.deposits.submit", "support.view", "guide.view", "user.apk.view", "user.source_events.view", "user.bank_upi.view", "user.bank_upi.submit", "user.bank_upi.update", "user.holds.view", "user.payin_commission.view", "user.payout.view", "user.payout.claim", "user.payout.submit", "user.commission.view", "user.commission_withdrawal.view", "user.commission_withdrawal.create", "user.commission_withdrawal.cancel", ...SELF_SECURITY]),
  merchant: Object.freeze(["profile.view", "profile.update", "merchant.overview.view", "support.view", "merchant.api_docs.view", "merchant.source_events.view", "merchant.ledger.view", "merchant.fees.view", "merchant.holds.view", "merchant.gateway.view", "merchant.gateway.create", "merchant.gateway.manage", "merchant.payout.view", "merchant.payout.create", "merchant.payout.review", "merchant.settlement.view", "merchant.settlement.create", ...SELF_SECURITY]),
  super_admin: Object.freeze([...ADMIN_OPERATIONS,"profile.view", "overview.view", "deposits.view", "deposits.review", "deposits.approve", "deposits.reject", "users.view", "users.approve", "users.reject", "merchants.view", "merchants.approve", "merchants.reject", "apk.view", "bank_upi.view", "bank_upi.review", "bank_upi.approve", "bank_upi.reject", "bank_upi.freeze", "bank_upi.release", "assignments.view", "assignments.update", "routing.view", "ledger.view", "holds.view", "holds.update", "transactions.view", "webhooks.view", "payout_operations.view", "payout_operations.resolve","payout_operations.proof", "payout_operations.capability", "commission_withdrawal.view", "commission_withdrawal.approve", "commission_hold.view", "commission_hold.manage", ...SELF_SECURITY]),
...
class AuthService {
  constructor(repository, options = {}) {
    ...
    if(options.statementEvidenceVerifier){
      const statements=new (require('../../integrations/statement-evidence').StatementEvidence)({pool:repository.pool,lookup:options.statementEvidenceVerifier,allowSynthetic:options.syntheticGatewayEvidence===true}),normal=options.paymentEvidenceVerifier;
    ...
    this.operations=new Operations({source:options.operationalSource,bridge:options.pairingBridge,crypto:options.mfaCrypto,onboarding:this.onboarding.workflow,legacyReader:options.legacyReader});
  ...
  async authenticated(session, operation, offset = 0, body = {}, entryRole) {
    ...
    let prepared;
    if (["payout/bulk","payout/submit"].includes(operation)) {
    ...
    const invoke = () => this.repository.withSession(sessionDigest, async (client, row) => {
      ...
      if (operation === "navigation") {
        ...
        return { groups: operationNavigation(payoutApi.navigation(gatewayApi.navigation(onboardingNavigation(businessNavigation(groups,context),context),context),context),context) };
      }
    ...
    for(let attempt=0;;attempt++){
      try { result=await invoke(); break; }
        ...
        // Retry only a fully rolled-back business transaction, re-reading the
        // approval, MFA and permission snapshot. Never reuse a stale principal.
        if(!/^(business|funding|onboarding|gateway|payout|operations)\//.test(operation) || !['40001','40P01'].includes(error.code) || attempt>=2)throw error;
      }
    }
