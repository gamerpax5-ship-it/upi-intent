  ...
  async authenticated(session, operation, offset = 0, body = {}, entryRole) {
    ...
    const invoke = () => this.repository.withSession(sessionDigest, async (client, row) => {
      ...
      if (operation === "navigation") {
        ...
        const result=require('../../panels/navigation').navigation(operationNavigation(payoutApi.navigation(gatewayApi.navigation(onboardingNavigation(businessNavigation(groups,context),context),context),context),context),context);
