...
const DEFAULT_GRANTS = Object.freeze({
  user: Object.freeze(["profile.view", "user.overview.view", "user.deposits.view", "user.deposits.submit", "support.view", "guide.view", "user.apk.view", "user.source_events.view", "user.bank_upi.view", "user.bank_upi.submit", "user.bank_upi.update", "user.holds.view", "user.payin_commission.view", ...SELF_SECURITY]),
  merchant: Object.freeze(["profile.view", "profile.update", "merchant.overview.view", "support.view", "merchant.api_docs.view", "merchant.source_events.view", "merchant.ledger.view", "merchant.fees.view", "merchant.holds.view", "merchant.gateway.view", "merchant.gateway.create", "merchant.gateway.manage", ...SELF_SECURITY]),
  super_admin: Object.freeze(["profile.view", "overview.view", "deposits.view", "deposits.review", "deposits.approve", "deposits.reject", "users.view", "users.approve", "users.reject", "merchants.view", "merchants.approve", "merchants.reject", "apk.view", "bank_upi.view", "bank_upi.review", "bank_upi.approve", "bank_upi.reject", "bank_upi.freeze", "bank_upi.release", "assignments.view", "assignments.update", "routing.view", "ledger.view", "holds.view", "holds.update", "transactions.view", "webhooks.view", ...SELF_SECURITY]),
  ...
  async authenticated(session, operation, offset = 0, body = {}) {
    ...
    const invoke = () => this.repository.withSession(sessionDigest, async (client, row) => {
      ...
      if (operation === "navigation") {
        ...
        return { groups: gatewayApi.navigation(onboardingNavigation(businessNavigation(groups,context),context),context) };
      }
    ...
    for(let attempt=0;;attempt++){
      try { result=await invoke(); break; }
        ...
        if(!/^(business|funding|onboarding|gateway)\//.test(operation) || !['40001','40P01'].includes(error.code) || attempt>=2)throw error;
      }
    }
