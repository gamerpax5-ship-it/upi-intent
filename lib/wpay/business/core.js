...
class BusinessCore {
 ...
 async candidates(client,merchantId){
  ...
  for(const row of rows){if(!summaries.has(row.user_id))summaries.set(row.user_id,await ledger.summary(client,row.user_id));const summary=summaries.get(row.user_id);
   const volume=row.bank_id?(await client.query(`WITH identity AS (SELECT account_key FROM wpay_auth.business_bank_identities WHERE bank_id=$1 AND version=$2)
    ...
    (SELECT COALESCE(sum(f.amount_minor),0) FROM wpay_auth.business_financial_events f JOIN wpay_auth.business_reservations r ON r.id=f.reservation_id JOIN wpay_auth.business_bank_identities i ON i.bank_id=r.bank_id AND i.version=r.bank_version WHERE i.account_key=(SELECT account_key FROM identity) AND f.created_at>CURRENT_TIMESTAMP-interval '24 hours'))::text AS used,
    (SELECT min(v.limit_minor)::text FROM wpay_auth.business_bank_accounts b JOIN wpay_auth.business_bank_versions v ON v.bank_id=b.id AND v.version=b.version JOIN wpay_auth.business_bank_identities i ON i.bank_id=v.bank_id AND i.version=v.version WHERE i.account_key=(SELECT account_key FROM identity) AND NOT b.deactivated) AS shared_limit`,[row.bank_id,row.version])).rows[0]:{used:'0',shared_limit:'0'};
