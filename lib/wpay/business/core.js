...
class BusinessCore {
 constructor({verifyEvidence=null,adminManagedCollections=process.env.WPAY_ADMIN_MANAGED_COLLECTIONS==='true'}={}){this.verifyEvidence=verifyEvidence;this.adminManagedCollections=adminManagedCollections;}
 ...
 async saveBank(client,ownerId,{bankId=null,version=null,details},actorId=ownerId){
  ...
  await client.query('INSERT INTO wpay_auth.business_bank_identities(bank_id,version,account_key) VALUES($1,$2,$3)',[id,next,require('../onboarding/evidence').accountDigest(fields)]);
 ...
 async candidates(client,merchantId){
  const rows=(await client.query(`SELECT x.id AS assignment_id,x.user_id,x.priority,x.min_minor::text,x.max_minor::text,x.status AS assignment_status,x.effective_from,
   a.status,a.tenant_id,e.approval_status,e.operations_enabled,e.initial_deposit_satisfied,(e.initial_deposit_satisfied OR EXISTS(SELECT 1 FROM wpay_auth.payout_claims pc WHERE pc.user_id=a.id AND pc.state='consumed' AND NOT EXISTS(SELECT 1 FROM wpay_auth.payout_disputes pd JOIN wpay_auth.payout_dispute_resolutions pr ON pr.dispute_id=pd.id WHERE pd.payout_id=pc.payout_id AND pr.outcome='payment_invalid')) OR EXISTS(SELECT 1 FROM wpay_auth.parking_locks pl WHERE pl.user_id=a.id AND pl.state='completed')) AS capacity_origin_ready,COALESCE((SELECT unlimited_collection FROM wpay_auth.user_collection_permissions cp WHERE cp.user_id=a.id),false) AS unlimited_collection,
   COALESCE((SELECT admin_managed_collections FROM wpay_auth.admin_commercial_defaults d WHERE d.tenant_id=a.tenant_id),$2::boolean) AS admin_managed_collections,
   ...
   (NOT z.enabled OR (z.factor_version>0 AND z.encrypted_secret IS NOT NULL)) AS security_ready,
   ...
   LEFT JOIN wpay_auth.business_bank_accounts b ON b.owner_id=a.id AND (x.bank_id=b.id OR (x.bank_id IS NULL AND NOT EXISTS(SELECT 1 FROM wpay_auth.admin_bank_approvals origin WHERE origin.bank_id=b.id)))
   LEFT JOIN wpay_auth.business_bank_versions v ON v.bank_id=b.id AND v.version=b.version WHERE x.merchant_id=$1 ORDER BY x.id,b.id`,[merchantId,this.adminManagedCollections])).rows;
  ...
  for(const row of rows){if(!summaries.has(row.user_id))summaries.set(row.user_id,await ledger.summary(client,row.user_id));const summary=summaries.get(row.user_id);
   const volume=row.bank_id?await require('./collection-policy').collectionVolume(client,row.bank_id,row.version):{used:'0',shared_limit:'0'};
   ...
   result.push({routeId:row.bank_id||row.assignment_id,userId:row.user_id,assignmentId:row.assignment_id,bankId:row.bank_id,bankVersion:row.version,
    priority:row.priority,minMinor:row.min_minor,maxMinor:row.max_minor,userStatus:row.status,approval:row.approval_status,mfaEnabled:row.enabled,authenticatorRequired:row.enabled,securityReady:row.security_ready,deviceRequired:row.device_required,deviceEligible:row.device_eligible,
    operationsEnabled:row.operations_enabled&&(row.admin_approved===true||row.statement_satisfied),statementRequired:row.admin_approved!==true&&!row.statement_satisfied,funded:row.capacity_origin_ready&&BigInt(summary.available)>0n,unlimitedCollection:row.unlimited_collection===true,
    ...
    adminManagedCollection:row.admin_managed_collections===true&&row.admin_approved===true,bankAdminApproved:row.admin_approved===true,bankVerified:!!row.bank_id&&row.verified_version===row.version,bankFrozen:row.frozen,bankDeactivated:row.deactivated,bankRemainingMinor:(remaining>0n?remaining:0n).toString(),
 ...
 async reserve(client,merchantId,{orderReference,idempotencyKey,amountMinor,ttlSeconds=300},actorId=merchantId){
  ...
  snapshot.route={assignmentId:route.assignmentId,bankId:route.bankId,bankVersion:route.bankVersion,priority:route.priority,...(route.adminManagedCollection?{fundingPolicy:'admin_managed'}:route.unlimitedCollection?{fundingPolicy:'user_unlimited'}:{})};
  ...
  await this.reservationEvent(client,row,'pending',actorId,'Eligibility evaluation');await this.reservationEvent(client,row,'active',actorId,route.adminManagedCollection?'Admin-managed collection; owner exposure tracked without collateral; no payment created':'Capacity reserved; no payment created');return this.publicReservation(row);
 }
