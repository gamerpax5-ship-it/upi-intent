...
class BusinessCore {
 ...
 async saveBank(client,ownerId,{bankId=null,version=null,details},actorId=ownerId){
  ...
  else if((await client.query('SELECT count(*)::integer AS n FROM wpay_auth.business_bank_accounts WHERE owner_id=$1 AND NOT deactivated',[ownerId])).rows[0].n>=20)throw new AuthError('RATE_LIMITED');
 ...
 async transitionBank(client,bankId,version,action,actorId,{ownerId=null,reason=''}={}){
  ...
  switch(action){
   ...
   case 'run':if(bank.status!=='enabled'||frozen||verified!==version)fail();await this.account(client,bank.owner_id,'user');state='running';break;
 ...
 async candidates(client,merchantId){
  const rows=(await client.query(`SELECT x.id AS assignment_id,x.user_id,x.priority,x.min_minor::text,x.max_minor::text,x.status AS assignment_status,x.effective_from,
   a.status,e.approval_status,e.operations_enabled,e.initial_deposit_satisfied,e.statement_satisfied,z.enabled,
   EXISTS(SELECT 1 FROM wpay_auth.sessions s WHERE s.account_id=a.id AND s.revoked_at IS NULL AND s.expires_at>CURRENT_TIMESTAMP
    AND s.last_seen_at>CURRENT_TIMESTAMP-interval '30 minutes' AND s.mfa_at IS NOT NULL AND s.session_epoch=a.session_epoch
    AND s.permission_version=a.permission_version AND s.security_version=z.security_version AND s.factor_version=z.factor_version) AS session_ready,
   ...
   LEFT JOIN wpay_auth.business_bank_accounts b ON b.owner_id=a.id
  ...
  for(const row of rows){if(!summaries.has(row.user_id))summaries.set(row.user_id,await ledger.summary(client,row.user_id));const summary=summaries.get(row.user_id);
   const bankUsage=row.bank_id?BigInt((await client.query(`SELECT ((SELECT COALESCE(sum(amount_minor),0) FROM wpay_auth.business_reservations WHERE bank_id=$1 AND state='active' AND expires_at>CURRENT_TIMESTAMP)+
    (SELECT COALESCE(sum(amount_minor),0) FROM wpay_auth.business_financial_events WHERE bank_id=$1 AND created_at>CURRENT_TIMESTAMP-interval '24 hours'))::text AS amount`,[row.bank_id])).rows[0].amount):0n;
   const remaining=BigInt(row.limit_minor||'0')-bankUsage;
   result.push({routeId:row.bank_id||row.assignment_id,userId:row.user_id,assignmentId:row.assignment_id,bankId:row.bank_id,bankVersion:row.version,
    priority:row.priority,minMinor:row.min_minor,maxMinor:row.max_minor,userStatus:row.status,approval:row.approval_status,mfaEnabled:row.enabled,sessionReady:row.session_ready,
    ...
    bankVerified:!!row.bank_id&&row.verified_version===row.version,bankFrozen:row.frozen,bankDeactivated:row.deactivated,bankRemainingMinor:(remaining>0n?remaining:0n).toString(),
 ...
 async confirmedPayin(client,input){
  ...
  if(r.state!=='active'){
   if(proof.source!=='statement_recovered'||!['expired','released'].includes(r.state))fail();
   if(BigInt((await ledger.summary(client,r.user_id)).available)<money.minor(r.amount_minor))throw new AuthError('INSUFFICIENT_CAPACITY');
  }
  ...
  await client.query("UPDATE wpay_auth.business_reservations SET state='consumed' WHERE id=$1",[r.id]);await this.reservationEvent(client,r,'consumed',null,proof.source);return {journalId,source:proof.source,commissionMinor:commission,feeMinor:fee,alreadyAccounted:false};
 }
 async hold(client,actorId,{id,ownerId,amountMinor,reference,reason,release}){
  ...
  if(prior&&(prior.owner_id!==ownerId||prior.amount_minor!==amountMinor||prior.reference!==reference))fail();
  if(release){if(!prior)fail();if(prior.state==='released')return {id,state:'released'};await client.query("UPDATE wpay_auth.business_holds SET state='released',released_at=CURRENT_TIMESTAMP WHERE id=$1",[id]);}
   ...
   await client.query("INSERT INTO wpay_auth.business_holds(id,owner_id,domain,amount_minor,currency,state,reason,reference,actor_id) VALUES($1,$2,$3,$4,'INR','active',$5,$6,$7)",[id,ownerId,domain,amountMinor,reason,reference,actorId]);}
  await ledger.post(client,{key:(release?'hold-release:':'hold:')+id,referenceType:'hold',referenceId:reference,actorId,metadata:{reason},entries:ledger.pair(ownerId,type,amountMinor,'INR',release?'debit':'credit')});
  await ledger.audit(client,{actorId,ownerId,entityId:id,event:release?'hold_released':'hold_created',reason});return {id,state:release?'released':'active'};
 }
