...
class BusinessApi {
 ...
 async run(client,row,context,operation,body){
  ...
  if(operation==='business/summary'){
   ...
   if(isUser)return {currency:b.currency,allocated:b.allocated,reserved:b.reserved,consumed:b.consumed,held:b.held,available:b.available,commission:b.commission,financialOperationsEnabled:false};
  ...
  if(operation==='business/banks'){
   ...
   return {banks:rows,actions:allowed,verificationConnected:false};
  }
  ...
  if(['business/banks/transition','business/banks/review'].includes(operation)){
   ...
   }else {if(!admin||!['review','approve','reject','freeze','release_freeze'].includes(body.action))throw new AuthError('FORBIDDEN');permit(context,'bank_upi.'+(body.action==='release_freeze'?'release':body.action),resource(owner,bank.id,bank.id));}
  ...
  if(operation==='business/routing'){
   ...
   return {candidates,reservations,strategy:routing.strategy,livePaymentsEnabled:false};
  }
