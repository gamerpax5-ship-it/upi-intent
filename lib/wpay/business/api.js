...
const POST=[...adminUpi.POST,'business/user-access/update','business/banks/daily-limit','business/banks/save','business/banks/transition','business/banks/review','business/assignments/update','business/reservations/reserve','business/reservations/release','business/ledger/search','business/ledger/adjust','business/holds/update'];
...
class BusinessApi {
 ...
 async run(client,row,context,operation,body){
  ...
  if(['business/user-access','business/user-access/update'].includes(operation)){
   ...
   const scope=permit(context,'users.commercial.update');
   ...
   return {users:(await client.query("SELECT a.id,a.name,COALESCE(p.free_setup,false) AS free_setup,COALESCE(p.unlimited_collection,false) AS unlimited_collection,p.reason FROM wpay_auth.accounts a LEFT JOIN wpay_auth.user_collection_permissions p ON p.user_id=a.id WHERE a.tenant_id=ANY($1) AND a.account_type='user' ORDER BY a.name,a.id LIMIT 200",[scope.tenantIds])).rows};
  }
  ...
  if(['business/ledger','business/ledger/search'].includes(operation)){
   ...
   const result=await client.query(`SELECT e.id,e.owner_id,a.account_type,e.ledger_type,e.direction,e.amount_minor::text,e.currency,j.reference_type,j.reference_id,j.idempotency_key,j.created_at,j.actor_source,j.snapshot,p.state AS payout_status
    FROM wpay_auth.business_entries e JOIN wpay_auth.business_journals j ON j.id=e.journal_id JOIN wpay_auth.accounts a ON a.id=e.owner_id LEFT JOIN wpay_auth.payout_orders p ON p.id::text=j.reference_id AND p.merchant_id=e.owner_id
