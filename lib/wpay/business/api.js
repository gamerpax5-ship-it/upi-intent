...
class BusinessApi {
 ...
 async run(client,row,context,operation,body){
  ...
  if(['business/user-access','business/user-access/update'].includes(operation)){
   ...
   const users=(await client.query(`SELECT a.id,a.name,COALESCE(p.free_setup,false) AS free_setup,COALESCE(p.unlimited_collection,false) AS unlimited_collection,p.reason,
    GREATEST(
      COALESCE((SELECT sum(CASE WHEN e.direction='credit' THEN e.amount_minor ELSE -e.amount_minor END) FROM wpay_auth.business_entries e WHERE e.owner_id=a.id AND e.currency='INR' AND e.ledger_type='capacity_allocated'),0)
      -COALESCE((SELECT sum(CASE WHEN e.direction='credit' THEN e.amount_minor ELSE -e.amount_minor END) FROM wpay_auth.business_entries e WHERE e.owner_id=a.id AND e.currency='INR' AND e.ledger_type='capacity_consumed'),0)
      -COALESCE((SELECT sum(CASE WHEN e.direction='credit' THEN e.amount_minor ELSE -e.amount_minor END) FROM wpay_auth.business_entries e WHERE e.owner_id=a.id AND e.currency='INR' AND e.ledger_type='capacity_hold'),0)
      -COALESCE((SELECT sum(r.amount_minor) FROM wpay_auth.business_reservations r WHERE r.user_id=a.id AND r.state='active' AND r.expires_at>CURRENT_TIMESTAMP),0)
    ,0)::text AS available_minor
    FROM wpay_auth.accounts a LEFT JOIN wpay_auth.user_collection_permissions p ON p.user_id=a.id
    WHERE a.tenant_id=ANY($1) AND a.account_type='user' ORDER BY a.name,a.id LIMIT 200`,[scope.tenantIds])).rows;
   return {users};
  }
