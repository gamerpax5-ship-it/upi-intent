...
async function overview(c,userId){
 ...
 FROM wpay_auth.payout_claims cl JOIN wpay_auth.payout_orders p ON p.id=cl.payout_id WHERE cl.user_id=$1 AND NOT EXISTS(SELECT 1 FROM wpay_auth.payout_disputes d JOIN wpay_auth.payout_dispute_resolutions dr ON dr.dispute_id=d.id WHERE d.payout_id=p.id AND dr.outcome='payment_invalid')`,[userId])).rows[0];
 ...
 UNION ALL SELECT (p.completed_at AT TIME ZONE 'Asia/Kolkata')::date,0,p.amount_minor FROM wpay_auth.payout_orders p JOIN wpay_auth.payout_claims cl ON cl.payout_id=p.id WHERE cl.user_id=$1 AND cl.state='consumed' AND NOT EXISTS(SELECT 1 FROM wpay_auth.payout_disputes d JOIN wpay_auth.payout_dispute_resolutions dr ON dr.dispute_id=d.id WHERE d.payout_id=p.id AND dr.outcome='payment_invalid') AND p.completed_at>=CURRENT_TIMESTAMP-interval '7 days'
