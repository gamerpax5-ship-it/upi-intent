...
async function run(core,c,row,context,operation,b={}){
 ...
 if(operation==='business/admin-upi'){
  ...
  const merchantCandidates=new Map();for(const route of routes){if(!merchantCandidates.has(route.merchant_id))merchantCandidates.set(route.merchant_id,await core.candidates(c,route.merchant_id));const candidate=merchantCandidates.get(route.merchant_id).find(x=>x.assignmentId===route.id&&x.bankId===route.bank_id);route.readiness=candidate?require('./routing').eligibility(candidate,route.min_minor,candidate.databaseNow):{eligible:false,reasons:['bank_unavailable']};}
  ...
  return {adminManagedCollections:core.adminManagedCollections===true,banks:visible,routes,accounts,hasMore:banks.length>50,offset,canCreate:canUsePermission({...context,permissionId:'bank_upi.approve'}).allowed,canRoute:canUsePermission({...context,permissionId:'assignments.update'}).allowed};
 }
