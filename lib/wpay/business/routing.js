...
function eligibility(candidate,amount,now){
 ...
 add(candidate.mfaEnabled===true&&(candidate.securityReady===undefined?candidate.sessionReady===true:candidate.securityReady===true),'mfa_required');
 ...
 add(candidate.bankApproved===true&&candidate.bankVerified===true&&['enabled','running'].includes(candidate.bankStatus)&&!candidate.bankFrozen&&!candidate.bankDeactivated,'bank_unavailable');
