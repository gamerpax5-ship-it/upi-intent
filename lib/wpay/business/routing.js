...
function eligibility(candidate,amount,now){
 const adminManaged=candidate.adminManagedCollection===true&&candidate.bankAdminApproved===true&&candidate.bankApproved===true;
 ...
 add(adminManaged||candidate.deviceRequired!==true||candidate.deviceEligible===true,'device_required');
 add(candidate.operationsEnabled===true,candidate.statementRequired===true?'statement_required':'operations_disabled');add(adminManaged||candidate.unlimitedCollection===true||candidate.funded===true,'funding_required');
 ...
 add(adminManaged||candidate.unlimitedCollection===true||minor(amount)<=minor(candidate.availableMinor,{zero:true}),'capacity_insufficient');
