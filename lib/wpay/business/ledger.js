...
function pair(ownerId,type,amount,currency='INR',direction='credit'){
 const book={capacity_allocated:'capacity',capacity_consumed:'capacity',capacity_reserved:'reservations',capacity_hold:'holds',user_commission:'commission',merchant_hold:'holds',merchant_payout_reserved:'reservations',merchant_settlement_reserved:'reservations',user_payout_commission:'commission',user_commission_adjustment:'commission',user_commission_hold:'commission',user_commission_reserved:'commission',user_commission_withdrawn:'commission'}[type]||'cash';
...
async function summary(client,ownerId){
 ...
 const gross=b.merchant_gross||0n,fees=b.merchant_platform_fee||0n,payoutFees=b.merchant_payout_fee||0n,merchantHeld=b.merchant_hold||0n,settlementReserved=b.merchant_settlement_reserved||0n,settlementPrincipal=b.merchant_settlement_principal||0n;
 const signed=allocated-consumed-reserved-held,deficit=signed<0n?-signed:0n;
 return {currency:'INR',allocated:allocated.toString(),reserved:reserved.toString(),consumed:consumed.toString(),held:held.toString(),available:(signed>0n?signed:0n).toString(),signedAvailable:signed.toString(),deficit:deficit.toString(),commission:(b.user_commission||0n).toString(),gross:gross.toString(),fees:fees.toString(),payoutFees:payoutFees.toString(),merchantHeld:merchantHeld.toString(),merchantPayoutReserved:(b.merchant_payout_reserved||0n).toString(),merchantPayoutPrincipal:(b.merchant_payout_principal||0n).toString(),merchantSettlementReserved:settlementReserved.toString(),merchantSettlementPrincipal:settlementPrincipal.toString(),merchantAvailable:(gross-fees-payoutFees+(b.merchant_adjustment||0n)-merchantHeld-(b.merchant_payout_reserved||0n)-(b.merchant_payout_principal||0n)-settlementReserved-settlementPrincipal).toString()};
}
