...
async function overview(c,row,context,body={}){
 ...
 if(has('bank_upi.view'))result.approvals.bank=(await c.query("SELECT count(*)::int count FROM wpay_auth.business_bank_accounts b JOIN wpay_auth.accounts a ON a.id=b.owner_id WHERE a.tenant_id=ANY($1) AND b.status IN ('submitted','review') AND NOT b.deactivated",[tenants])).rows[0].count;
