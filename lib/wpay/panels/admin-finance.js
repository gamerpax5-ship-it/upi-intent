...
async function run(c,row,context,operation,b){
 ...
 if(operation==='panel/expense/create'){
  ...
  permit(context,'ledger.adjust',{kind:'record',id:row.id,tenantId:b.tenantId,ownerType:'principal',ownerId:row.id});
 ...
 if(operation==='panel/expense/void'){
  ...
  permit(context,'ledger.adjust',{kind:'record',id:e.id,tenantId:e.tenant_id,ownerType:'principal',ownerId:row.id});
 ...
 const payout=(await c.query(`SELECT COALESCE(sum(p.fixed_fee_minor),0)::text fixed,COALESCE(sum(p.percentage_fee_minor),0)::text percentage,count(*)::int count FROM wpay_auth.payout_orders p JOIN wpay_auth.accounts a ON a.id=p.merchant_id WHERE a.tenant_id=ANY($1) AND p.state='successful' AND p.completed_at>=$2 AND p.completed_at<$3`,[tenants,from,to])).rows[0];
 ...
 return {from,to,rows,fees,payout,funding,settlement,expenseTotals,totalCosts,operatingMargin:margin(fees,totalCosts),expenses:expenses.slice(0,50),hasMore:expenses.length>50,tenants,canManage:canUsePermission({...context,permissionId:'ledger.adjust'}).allowed,fxProfit:null};
}
