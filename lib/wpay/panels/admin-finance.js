...
async function run(c,row,context,operation,b){
 ...
 return {from,to,rows,fees,payout,funding,settlement,expenseTotals,totalCosts,operatingMargin:margin(fees,totalCosts),expenses:expenses.slice(0,50),hasMore:expenses.length>50,tenants,canManage:canUsePermission({...context,permissionId:'finance_expenses.manage'}).allowed,fxProfit:null};
}
