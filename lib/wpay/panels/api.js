...
const GET=['panel/admin-finance','panel/admin-audit','panel/admin-overview','panel/fees','panel/profile','panel/support','panel/notifications','panel/settings','panel/webhooks','panel/credentials','panel/api-logs','panel/devices','panel/analytics','panel/reports'];
const POST=[...GET,'panel/expense/create','panel/expense/void','panel/profile/update','panel/preferences','panel/support/create','panel/support/update','panel/directory','panel/directory/create','panel/directory/update','panel/credentials/create','panel/credentials/revoke','panel/reports/export'];
...
async function directory(c,row,context,body,fixedCurrency){
 ...
 const prefix=body.type==='user'?'users':'merchants';return {...more(rows,f),actions:['approve','reject','suspend','commercial.update'].filter(p=>canUsePermission({...context,permissionId:prefix+'.'+p}).allowed),canCreate:['admin','super_admin'].includes(row.account_type)&&canUsePermission({...context,permissionId:prefix+'.approve'}).allowed,fixedFeeCurrency:fixedCurrency||null};
}
