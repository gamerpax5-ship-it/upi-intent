...
const POST=[...GET,'panel/merchant-default-rate/update','panel/expense/create','panel/expense/void','panel/profile/update','panel/preferences','panel/notifications/read-all','panel/support/create','panel/support/update','panel/directory','panel/directory/create','panel/directory/update','panel/credentials/create','panel/credentials/revoke','panel/webhooks/configure','panel/webhooks/retry','panel/reports/export'];
...
async function run(c,row,context,operation,b,fixedCurrency,gateway){
 ...
 if(operation==='panel/merchant-default-rate'){
  if(!['admin','super_admin'].includes(row.account_type))fail();const scope=permit(context,'merchants.view'),rows=(await c.query("SELECT tenant_id,merchant_inr_per_usdt,merchant_fixed_payout_fee,merchant_payin_fee,merchant_payout_fee,merchant_payment_link_ttl_seconds,admin_managed_collections,updated_at FROM wpay_auth.admin_commercial_defaults WHERE tenant_id=ANY($1) ORDER BY tenant_id",[scope.tenantIds])).rows,map=new Map(rows.map(x=>[x.tenant_id,x]));
  return {tenants:scope.tenantIds.map(id=>{const x=map.get(id),fallback=process.env.WPAY_ADMIN_MANAGED_COLLECTIONS==='true';return {tenantId:id,rate:x?.merchant_inr_per_usdt||'107',fixedPayoutFee:x?.merchant_fixed_payout_fee||'6',payinFee:x?.merchant_payin_fee||'1.2',payoutFee:x?.merchant_payout_fee||'0.8',paymentLinkTtlSeconds:x?.merchant_payment_link_ttl_seconds||300,adminManagedCollections:x?.admin_managed_collections??fallback,adminManagedCollectionsSource:x?.admin_managed_collections===null||x?.admin_managed_collections===undefined?'environment':'admin',updatedAt:x?.updated_at||null};}),canUpdate:canUsePermission({...context,permissionId:'merchants.commercial.update'}).allowed};
 }
 if(operation==='panel/merchant-default-rate/update'){
  if(!['admin','super_admin'].includes(row.account_type))fail();validation.exactFields(b,['tenantId','rate','fixedPayoutFee','payinFee','payoutFee','paymentLinkTtlSeconds','adminManagedCollections']);recent(row);const scope=permit(context,'merchants.commercial.update');if(typeof b.tenantId!=='string'||!scope.tenantIds.includes(b.tenantId))fail();
  const decimal=require('../auth/runtime/commercial').decimal,rate=decimal(b.rate,{positive:true}),fixedPayoutFee=decimal(b.fixedPayoutFee),payinFee=decimal(b.payinFee,{percentage:true}),payoutFee=decimal(b.payoutFee,{percentage:true}),ttl=Number(b.paymentLinkTtlSeconds);if(!Number.isInteger(ttl)||ttl<30||ttl>900||typeof b.adminManagedCollections!=='boolean')fail('INVALID_INPUT');
  await c.query(`INSERT INTO wpay_auth.admin_commercial_defaults(tenant_id,merchant_inr_per_usdt,merchant_fixed_payout_fee,merchant_payin_fee,merchant_payout_fee,merchant_payment_link_ttl_seconds,admin_managed_collections,actor_id,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,CURRENT_TIMESTAMP)
   ON CONFLICT(tenant_id) DO UPDATE SET merchant_inr_per_usdt=EXCLUDED.merchant_inr_per_usdt,merchant_fixed_payout_fee=EXCLUDED.merchant_fixed_payout_fee,merchant_payin_fee=EXCLUDED.merchant_payin_fee,merchant_payout_fee=EXCLUDED.merchant_payout_fee,merchant_payment_link_ttl_seconds=EXCLUDED.merchant_payment_link_ttl_seconds,admin_managed_collections=EXCLUDED.admin_managed_collections,actor_id=EXCLUDED.actor_id,updated_at=CURRENT_TIMESTAMP`,[b.tenantId,rate,fixedPayoutFee,payinFee,payoutFee,ttl,b.adminManagedCollections,row.id]);
  await audit(c,row,row.id,'merchant_commercial_defaults_updated',{tenantId:b.tenantId,rate,fixedPayoutFee,payinFee,payoutFee,paymentLinkTtlSeconds:ttl,adminManagedCollections:b.adminManagedCollections});return {tenantId:b.tenantId,rate,fixedPayoutFee,payinFee,payoutFee,paymentLinkTtlSeconds:ttl,adminManagedCollections:b.adminManagedCollections};
 }
 ...
 if(operation==='panel/notifications'){
  const f=page(b);permit(context,'notifications.view');const preferences=(await c.query('SELECT locale,in_app_notifications,notifications_read_at FROM wpay_auth.preferences WHERE account_id=$1',[row.id])).rows[0];
  ...
  const marked=rows.map(x=>({...x,read:!!preferences.notifications_read_at&&+new Date(x.created_at)<=+new Date(preferences.notifications_read_at)}));
  return {...more(marked,f),preferences,emailDeliveryConfigured:false,canUpdate:canUsePermission({...context,permissionId:'notifications.update'}).allowed};
 }
 if(operation==='panel/notifications/read-all'){
  validation.exactFields(b,[]);permit(context,'notifications.update',self(row));const changed=(await c.query('UPDATE wpay_auth.preferences SET notifications_read_at=CURRENT_TIMESTAMP WHERE account_id=$1 RETURNING notifications_read_at',[row.id])).rows[0];await audit(c,row,row.id,'notifications_marked_read');return {readAt:changed.notifications_read_at};
 }
 ...
 if(operation==='panel/webhooks/configure'){
  validation.exactFields(b,['merchantId','url','enabled']);v.id(b.merchantId);if(typeof b.enabled!=='boolean')fail('INVALID_INPUT');recent(row);const scope=permit(context,'webhooks.view');
  ...
  const latest=(await c.query('SELECT id,url FROM wpay_auth.gateway_endpoints WHERE merchant_id=$1 ORDER BY created_at DESC,id DESC LIMIT 1',[target.id])).rows[0];
  let result;
  if(!latest||latest.url!==b.url)result=await gateway.configure(c,target.id,{url:b.url},row.id);
  else result={id:latest.id,url:latest.url,displayOnce:false,secret:null};
  await c.query(`INSERT INTO wpay_auth.gateway_endpoint_controls(merchant_id,enabled,actor_id,updated_at) VALUES($1,$2,$3,CURRENT_TIMESTAMP)
   ON CONFLICT(merchant_id) DO UPDATE SET enabled=EXCLUDED.enabled,actor_id=EXCLUDED.actor_id,updated_at=CURRENT_TIMESTAMP`,[target.id,b.enabled,row.id]);
  if(!b.enabled)await c.query("UPDATE wpay_auth.gateway_outbox SET state='unconfigured',lease_id=NULL,lease_until=NULL WHERE merchant_id=$1 AND state IN('pending','leased')",[target.id]);
  else await c.query("UPDATE wpay_auth.gateway_outbox SET state='pending',next_attempt_at=CURRENT_TIMESTAMP WHERE merchant_id=$1 AND state='unconfigured' AND endpoint_id IS NOT NULL",[target.id]);
  await audit(c,row,target.id,'webhook_configuration_updated',{endpointId:result.id,enabled:b.enabled,secretRotated:!!result.secret});return {...result,enabled:b.enabled};
 }
 ...
 let sql;
 if(operation==='panel/credentials')sql='SELECT k.id,k.merchant_id,a.name AS merchant_name,k.prefix,k.label,k.scopes,k.created_at,k.last_used_at,k.revoked_at FROM wpay_auth.gateway_keys k JOIN wpay_auth.accounts a ON a.id=k.merchant_id WHERE a.tenant_id=ANY($1) ORDER BY k.created_at DESC,k.id LIMIT $2 OFFSET $3';
 if(operation==='panel/webhooks')sql='SELECT o.id,o.merchant_id,a.name AS merchant_name,o.order_id,o.event_type,o.state,o.attempts,o.next_attempt_at,o.last_code,o.created_at FROM wpay_auth.gateway_outbox o JOIN wpay_auth.accounts a ON a.id=o.merchant_id WHERE a.tenant_id=ANY($1) ORDER BY o.created_at DESC,o.id LIMIT $2 OFFSET $3';
 if(operation==='panel/api-logs')sql='SELECT l.id,l.merchant_id,a.name AS merchant_name,l.operation,l.created_at FROM wpay_auth.api_access_audit l JOIN wpay_auth.accounts a ON a.id=l.merchant_id WHERE a.tenant_id=ANY($1) ORDER BY l.created_at DESC,l.id LIMIT $2 OFFSET $3';
 ...
 const extra={};
 if(operation==='panel/webhooks')extra.endpoints=more((await c.query(`SELECT DISTINCT ON(e.merchant_id) e.id,e.merchant_id,a.name AS merchant_name,e.url,e.created_at,COALESCE(ctrl.enabled,true) AS enabled
  FROM wpay_auth.gateway_endpoints e JOIN wpay_auth.accounts a ON a.id=e.merchant_id
  LEFT JOIN wpay_auth.gateway_endpoint_controls ctrl ON ctrl.merchant_id=e.merchant_id
  WHERE a.tenant_id=ANY($1) ORDER BY e.merchant_id,e.created_at DESC,e.id DESC LIMIT $2 OFFSET $3`,[scope.tenantIds,f.limit+1,f.offset])).rows,f);
