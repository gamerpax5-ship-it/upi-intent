...
async function run(c,row,context,operation,b,fixedCurrency,gateway){
 ...
 if(operation==='panel/merchant-default-rate'){
  ...
  return {tenants:scope.tenantIds.map(id=>{const x=map.get(id);return {tenantId:id,rate:x?.merchant_inr_per_usdt||'107',fixedPayoutFee:x?.merchant_fixed_payout_fee||'6',payinFee:x?.merchant_payin_fee||'1.2',payoutFee:x?.merchant_payout_fee||'0.8',paymentLinkTtlSeconds:x?.merchant_payment_link_ttl_seconds||300,updatedAt:x?.updated_at||null};}),adminManagedCollections:process.env.WPAY_ADMIN_MANAGED_COLLECTIONS==='true',canUpdate:canUsePermission({...context,permissionId:'merchants.commercial.update'}).allowed};
 }
 if(operation==='panel/merchant-default-rate/update'){
  if(!['admin','super_admin'].includes(row.account_type))fail();validation.exactFields(b,['tenantId','rate','fixedPayoutFee','payinFee','payoutFee','paymentLinkTtlSeconds']);recent(row);const scope=permit(context,'merchants.commercial.update');if(typeof b.tenantId!=='string'||!scope.tenantIds.includes(b.tenantId))fail();
  const decimal=require('../auth/runtime/commercial').decimal,rate=decimal(b.rate,{positive:true}),fixedPayoutFee=decimal(b.fixedPayoutFee),payinFee=decimal(b.payinFee,{percentage:true}),payoutFee=decimal(b.payoutFee,{percentage:true}),ttl=Number(b.paymentLinkTtlSeconds);if(!Number.isInteger(ttl)||ttl<30||ttl>900)fail('INVALID_INPUT');
  await c.query(`INSERT INTO wpay_auth.admin_commercial_defaults(tenant_id,merchant_inr_per_usdt,merchant_fixed_payout_fee,merchant_payin_fee,merchant_payout_fee,merchant_payment_link_ttl_seconds,actor_id,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,CURRENT_TIMESTAMP)
   ON CONFLICT(tenant_id) DO UPDATE SET merchant_inr_per_usdt=EXCLUDED.merchant_inr_per_usdt,merchant_fixed_payout_fee=EXCLUDED.merchant_fixed_payout_fee,merchant_payin_fee=EXCLUDED.merchant_payin_fee,merchant_payout_fee=EXCLUDED.merchant_payout_fee,merchant_payment_link_ttl_seconds=EXCLUDED.merchant_payment_link_ttl_seconds,actor_id=EXCLUDED.actor_id,updated_at=CURRENT_TIMESTAMP`,[b.tenantId,rate,fixedPayoutFee,payinFee,payoutFee,ttl,row.id]);
  await audit(c,row,row.id,'merchant_commercial_defaults_updated',{tenantId:b.tenantId,rate,fixedPayoutFee,payinFee,payoutFee,paymentLinkTtlSeconds:ttl});return {tenantId:b.tenantId,rate,fixedPayoutFee,payinFee,payoutFee,paymentLinkTtlSeconds:ttl};
 }
