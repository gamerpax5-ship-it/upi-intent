...
async function run(c,row,context,operation,b,fixedCurrency,gateway){
 ...
 if(operation==='panel/webhooks/configure'){
  ...
  let result=null;
  if(b.url){result=await gateway.configure(c,target.id,{url:b.url},row.id);}
  else{
   const latest=(await c.query('SELECT id,url FROM wpay_auth.gateway_endpoints WHERE merchant_id=$1 ORDER BY created_at DESC,id DESC LIMIT 1',[target.id])).rows[0];if(!latest)fail('CONFLICT');result={id:latest.id,url:latest.url,displayOnce:false,secret:null};
  }
  ...
  await audit(c,row,target.id,'webhook_configuration_updated',{endpointId:result.id,enabled:b.enabled});return {...result,enabled:b.enabled};
 }
