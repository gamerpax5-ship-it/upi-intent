...
const GET=['panel/profile','panel/support','panel/notifications','panel/settings','panel/webhooks','panel/credentials','panel/api-logs','panel/devices','panel/analytics','panel/reports'];
...
async function run(c,row,context,operation,b,fixedCurrency,gateway){
 ...
 if(['panel/analytics','panel/reports','panel/reports/export'].includes(operation)){
  if((operation==='panel/analytics')!==['user','merchant'].includes(row.account_type))fail();
