...
const GET=['panel/admin-overview','panel/fees','panel/profile','panel/support','panel/notifications','panel/settings','panel/webhooks','panel/credentials','panel/api-logs','panel/devices','panel/analytics','panel/reports'];
...
async function run(c,row,context,operation,b,fixedCurrency,gateway){
 ...
 if(operation==='panel/settings'){
  permit(context,'settings.view');return {adminLogin:'Email and password',customerEmployeeLogin:'Password and authenticator',sessionIdleMinutes:30,sessionMaximumHours:12,temporaryPasswordHours:24,resetChallengeMinutes:10,sensitiveActionConfirmationMinutes:5,securityPolicyEditable:false};
 }
