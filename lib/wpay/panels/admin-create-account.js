...
async function create(c,row,context,b){
 ...
 const scope=permit(context,b.type==='user'?'users.approve':'merchants.approve');recent(row);
 ...
 const id=randomUUID(),record=await hashPassword(b.password,b.type),grants=require('../auth/runtime/service').DEFAULT_GRANTS[b.type];
