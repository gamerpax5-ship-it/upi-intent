...
const POST=['parking/beneficiary/create','parking/beneficiary/confirm','parking/order/create','parking/lock','parking/release','parking/submit','parking/review','parking/proof'];
...
function preparation(context,row,operation){if(operation!=='parking/submit')return;if(row.account_type!=='user')fail();permit(context,'user.parking_payments.create',own(row,row.id,'create'));recent(row);}
