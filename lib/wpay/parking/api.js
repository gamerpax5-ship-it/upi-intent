...
async function run(core,client,row,context,operation,body,prepared){
 ...
 if(operation==='parking/admin'){if(!admin)fail();const scope=permit(context,'parking.view');return {...await core.admin(client,scope.tenantIds),tenants:scope.tenantIds,canCreate:canUsePermission({...context,permissionId:'parking.create'}).allowed};}
