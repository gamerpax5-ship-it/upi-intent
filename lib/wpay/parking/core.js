...
class Parking{
 ...
 async createBeneficiary(client,actorId,input){
  ...
  uuid(input.requestId);const tenantId=tenant(input.tenantId),details=this.details(input),digest=ledger.digest(details);
 ...
 async beneficiaries(client,{tenantId,userId=null}){
  const rows=(await client.query(`SELECT b.*,EXISTS(SELECT 1 FROM wpay_auth.parking_beneficiary_confirmations c WHERE c.beneficiary_id=b.id AND c.user_id=$2) AS confirmed
   FROM wpay_auth.parking_beneficiaries b WHERE b.tenant_id=$1 AND b.revoked_at IS NULL ORDER BY b.created_at DESC,b.id`,[tenantId,userId])).rows;
  return rows.map(r=>({...this.projectBeneficiary(r,true),confirmed:r.confirmed}));
 }
 ...
 async createOrder(client,actorId,input){
  const allowed=['requestId','tenantId','beneficiaryId','reference','totalMinor','minMinor'];if(!input||Object.keys(input).some(k=>!allowed.includes(k))||allowed.some(k=>!Object.hasOwn(input,k)))fail();
  uuid(input.requestId);const tenantId=tenant(input.tenantId);uuid(input.beneficiaryId);v.reference(input.reference);const total=money.minor(input.totalMinor),min=money.minor(input.minMinor);if(min<=0n||total<=0n||min>total)fail();
  ...
  const fingerprint=ledger.digest({tenantId,beneficiaryId:b.id,reference:input.reference,totalMinor:input.totalMinor,minMinor:input.minMinor,detailsDigest:b.details_digest});
  ...
  if(prior){if(ledger.digest(prior.snapshot)!==fingerprint)fail('CONFLICT');return this.projectOrder(prior,'0');}
  const id=randomUUID(),snapshot={tenantId,beneficiaryId:b.id,reference:input.reference,totalMinor:input.totalMinor,minMinor:input.minMinor,detailsDigest:b.details_digest},row=(await client.query(
 ...
 async expire(client,orderId=null){
  ...
  for(const l of active){await client.query("UPDATE wpay_auth.parking_locks SET state='cooldown',cooldown_until=CURRENT_TIMESTAMP+($2*interval '1 second') WHERE id=$1",[l.id,COOLDOWN_SECONDS]);await this.transition(client,l.id,l.user_id,'expired','Payment window expired');}
 ...
 async orders(client,{tenantId,userId=null}){
  await this.expire(client);const params=[tenantId,userId],rows=(await client.query(`SELECT o.*,b.encrypted_details,
   ...
   FROM wpay_auth.parking_orders o JOIN wpay_auth.parking_beneficiaries b ON b.id=o.beneficiary_id
  ...
  return rows.map(r=>({...this.projectOrder(r,r.locked),beneficiary:JSON.parse(this.crypto.open(r.encrypted_details,binding('beneficiary',r.beneficiary_id)))})).filter(r=>BigInt(r.remainingMinor)>0n);
 }
 ...
 async lock(client,userId,tenantId,input){
  ...
  await ledger.lock(client);await this.expire(client,input.orderId);
  ...
  const locked=BigInt(await this.lockedMinor(client,order.id)),remaining=BigInt(order.total_minor)-locked;if(amount<BigInt(order.min_minor)||amount>remaining)fail('INSUFFICIENT_CAPACITY');
 ...
 async submit(client,userId,body,proof){
  ...
  const row=(await client.query('SELECT * FROM wpay_auth.parking_locks WHERE id=$1 AND user_id=$2 FOR UPDATE',[body.id,userId])).rows[0];if(!row||row.state!=='active'||+row.expires_at<=Date.now())fail('CONFLICT');
  const previous=(await client.query('SELECT proof_digest FROM wpay_auth.parking_submissions WHERE parking_id=$1',[row.id])).rows[0];if(previous){if(previous.proof_digest!==proof.digest)fail('CONFLICT');return {id:row.id,state:'submitted'};}
 ...
 async userHistory(client,userId){
  await this.expire(client);const rows=(await client.query(`SELECT l.*,o.reference,o.beneficiary_id,b.encrypted_details,s.scan_state FROM wpay_auth.parking_locks l JOIN wpay_auth.parking_orders o ON o.id=l.order_id JOIN wpay_auth.parking_beneficiaries b ON b.id=o.beneficiary_id LEFT JOIN wpay_auth.parking_submissions s ON s.parking_id=l.id WHERE l.user_id=$1 ORDER BY l.created_at DESC,l.id LIMIT 100`,[userId])).rows;
  return rows.map(r=>({id:r.id,orderId:r.order_id,reference:r.reference,amountMinor:r.amount_minor,state:r.state,expiresAt:r.expires_at,cooldownUntil:r.cooldown_until,scanState:r.scan_state??null,beneficiary:JSON.parse(this.crypto.open(r.encrypted_details,binding('beneficiary',r.beneficiary_id)))}));
 }
 async admin(client,tenants){
  ...
  const orders=(await client.query('SELECT * FROM wpay_auth.parking_orders WHERE tenant_id=ANY($1) ORDER BY created_at DESC,id LIMIT 100',[tenants])).rows.map(r=>this.projectOrder(r,'0'));
