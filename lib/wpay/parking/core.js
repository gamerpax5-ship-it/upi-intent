...
class Parking{
 ...
 async admin(client,tenants){
  await this.expire(client);const beneficiaryRows=(await client.query(`SELECT b.*,
   (SELECT count(*)::int FROM wpay_auth.parking_beneficiary_confirmations c WHERE c.beneficiary_id=b.id) AS confirmation_count
   FROM wpay_auth.parking_beneficiaries b WHERE b.tenant_id=ANY($1) ORDER BY b.created_at DESC,b.id LIMIT 100`,[tenants])).rows;
  const beneficiaries=beneficiaryRows.map(r=>({...this.projectBeneficiary(r,true),confirmationCount:r.confirmation_count}));
