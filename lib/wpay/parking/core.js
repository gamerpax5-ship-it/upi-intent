...
class Parking{
 ...
 async prepare(operation,body){if(!['parking/submit','parking/late/open'].includes(operation))return null;if(!body||Object.getPrototypeOf(body)!==Object.prototype||!body.proof)fail();return uploads.proof(body.proof,this.scanner);}
 ...
 async lockedMinor(client,orderId){return (BigInt((await client.query("SELECT COALESCE(sum(amount_minor),0)::text AS n FROM wpay_auth.parking_locks WHERE order_id=$1 AND state=ANY($2)",[orderId,['active','cooldown','submitted','review','completed','disputed']])).rows[0].n)+await require('../reviews/late-payment').held(client,'parking',orderId)).toString();}
 async orders(client,{tenantId,userId=null}){
  ...
  for(const r of rows)r.locked=(BigInt(r.locked)+await require('../reviews/late-payment').held(client,'parking',r.id)).toString();
 ...
 async userHistory(client,userId){
  ...
  return rows.map(r=>({id:r.id,orderId:r.order_id,reference:r.reference,amountMinor:r.amount_minor,state:r.state,expiresAt:r.expires_at,cooldownUntil:r.cooldown_until,scanState:r.scan_state??null,reason:r.last_reason??null,canLateProof:['released','expired'].includes(r.state)&&+r.database_now>=+r.expires_at+COOLDOWN_SECONDS*1000,canSubmit:(r.state==='active'||r.state==='cooldown'&&+r.cooldown_until===+r.expires_at+COOLDOWN_SECONDS*1000)&&policy.claimWindow(r.created_at,r.database_now).canSubmit,beneficiary:JSON.parse(this.crypto.open(r.encrypted_details,binding('beneficiary',r.beneficiary_id)))}));
 }
 async admin(client,tenants){
  await this.expire(client);const beneficiaryRows=(await client.query(`SELECT b.*,actor.account_type AS actor_type,actor.name AS actor_name,
   (SELECT count(*)::int FROM wpay_auth.parking_beneficiary_confirmations c WHERE c.beneficiary_id=b.id) AS confirmation_count
   FROM wpay_auth.parking_beneficiaries b JOIN wpay_auth.accounts actor ON actor.id=b.actor_id
   WHERE b.tenant_id=ANY($1) ORDER BY b.created_at DESC,b.id LIMIT 100`,[tenants])).rows;
  const beneficiaries=beneficiaryRows.map(r=>({...this.projectBeneficiary(r,true),confirmationCount:r.confirmation_count,sourceType:r.actor_type,sourceName:r.actor_name}));
  ...
  const reviewRows=(await client.query(`SELECT l.*,o.reference,o.tenant_id,o.beneficiary_id,s.scan_state,s.created_at AS submitted_at,s.encrypted_evidence,a.name AS user_name,
   (SELECT actor.name FROM wpay_auth.parking_transitions t LEFT JOIN wpay_auth.accounts actor ON actor.id=t.actor_id WHERE t.parking_id=l.id ORDER BY t.version DESC LIMIT 1) AS reviewer_name
   FROM wpay_auth.parking_locks l JOIN wpay_auth.parking_orders o ON o.id=l.order_id JOIN wpay_auth.accounts a ON a.id=l.user_id
   LEFT JOIN wpay_auth.parking_submissions s ON s.parking_id=l.id
   WHERE o.tenant_id=ANY($1) AND l.state=ANY($2) ORDER BY COALESCE(s.created_at,l.created_at),l.id LIMIT 100`,[tenants,['submitted','review','disputed']])).rows;
  const reviews=reviewRows.map(r=>{let utr=null;if(r.encrypted_evidence){try{utr=JSON.parse(this.crypto.open(r.encrypted_evidence,binding('evidence',r.id))).utr||null;}catch{utr=null;}}return {id:r.id,orderId:r.order_id,reference:r.reference,tenantId:r.tenant_id,beneficiaryId:r.beneficiary_id,userName:r.user_name,amountMinor:r.amount_minor,state:r.state,scanState:r.scan_state,submittedAt:r.submitted_at,utr,reviewer:r.reviewer_name||'—'};});
  for(const o of orders){const h=await require('../reviews/late-payment').held(client,'parking',o.id);o.remainingMinor=(BigInt(o.remainingMinor)>h?BigInt(o.remainingMinor)-h:0n).toString();}
  const confirmations=(await client.query(`SELECT c.beneficiary_id,c.user_id,a.name AS user_name
   FROM wpay_auth.parking_beneficiary_confirmations c
   JOIN wpay_auth.parking_beneficiaries b ON b.id=c.beneficiary_id
   JOIN wpay_auth.accounts a ON a.id=c.user_id
   WHERE b.tenant_id=ANY($1) ORDER BY c.created_at DESC,c.beneficiary_id,c.user_id`,[tenants])).rows;
  const users=(await client.query("SELECT id,name FROM wpay_auth.accounts a JOIN wpay_auth.eligibility e ON e.account_id=a.id WHERE a.tenant_id=ANY($1) AND a.account_type='user' AND a.status='active' AND e.approval_status='approved' ORDER BY a.name,a.id",[tenants])).rows;
  return {beneficiaries,orders,reviews,confirmations,users};
 }
