...
function navigation(groups,context){
 const user=context.principal.type==='user';const pages=user?[['devices','user.device_pairing.view','Linked Devices'],['otp','user.live_otp.view','OTP Events'],['transactions','user.transaction_history.view','Pay-in Transactions']]:[['devices','devices.view','Linked Devices'],['activation','devices.view','Activation Codes'],['otp','apk_otp_events.view_all','OTP Events'],['employees','employee_management.view','Employees'],['transactions','utr_center.view','UTR Center'],['pending-utrs','utr_center.view','Pending UTR'],['statements','statement_reconciliation.view','Statements']];
 ...
 const result=groups.map(g=>({...g,children:g.children.filter(child=>user||child.permissionId!=='devices.view')}));
