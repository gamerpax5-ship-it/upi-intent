...
function recent(row){const age=+new Date(row.database_now)-+new Date(row.mfa_at);if(!row.mfa_at||!row.database_now||!Number.isFinite(age)||age<0||age>300000)fail('RECENT_MFA_REQUIRED');}
...
function navigation(groups,context){
 const user=context.principal.type==='user';const pages=user?[['devices','user.device_pairing.view','Linked Devices'],['otp','user.live_otp.view','OTP Events'],['transactions','user.transaction_history.view','Pay-in Transactions']]:[['otp','apk_otp_events.view_all','OTP Events'],['employees','employee_management.view','Employees'],['transactions','utr_center.view','UTR Center'],['statements','statement_reconciliation.view','Statements']];
 ...
 for(const child of children){const name=child.destinationId.split('.').at(-1),section=name==='employees'?'employees':['transactions','statements'].includes(name)?'transactions':'apk_events';let group=result.find(g=>g.id.endsWith('.'+section));if(!group){group={id:'operations.group.'+section,label:{employees:'Employees',transactions:'Transactions',apk_events:'APK & Events'}[section],children:[]};result.push(group);}group.children.push(child);}
 return result;
}
