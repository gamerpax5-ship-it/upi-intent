...
async function run(client,row,context,operation,body){
 ...
 if(operation==='operations/employee/create'){
  ...
  await client.query('INSERT INTO wpay_auth.credentials(account_id,password_record) VALUES($1,$2)',[id,recordHash]);
  ...
  await record(client,row,id,1,selected.permissions,selected.tenants,'active');return {id,email,oneTimePassword:password,mfaRequired:true,loginPath:'/employee'};
 }
