"use strict";
const {randomUUID,randomBytes}=require('node:crypto'),validation=require('../auth/runtime/validation'),v=require('../business/validation'),{hashPassword}=require('../auth/runtime/passwords'),{validateGrants}=require('../authorization-policy'),{PERMISSION_CATALOG,getEmployeePermissionGroups}=require('../permission-catalog'),{fail,recent,permit,audit}=require('./access');
...
async function run(client,row,context,operation,body){
 ...
 if(operation==='operations/employees'){
  ...
  return {employees:result.slice(0,f.limit),hasMore:result.length>f.limit,offset:f.offset,tenantIds:scope.tenantIds,canCreate:require('../authorization-policy').canUsePermission({...context,permissionId:'employee_management.create'}).allowed,canUpdate:require('../authorization-policy').canUsePermission({...context,permissionId:'employee_management.update'}).allowed,permissions:PERMISSION_CATALOG.filter(p=>p.employeeDelegable&&!p.restricted&&context.grants.includes(p.id)).map(p=>({id:p.id,label:p.label,dependencies:p.dependencies})),permissionGroups:getEmployeePermissionGroups().map(g=>({...g,permissions:g.permissions.map(p=>({id:p.id,label:p.label,dependencies:p.dependencies,restricted:p.restricted,selectable:p.selectable&&context.grants.includes(p.id)}))})),requiredPermissions:SELF};
 }
 recent(row);
 ...
 if(!body||Object.getPrototypeOf(body)!==Object.prototype||Object.keys(body).some(k=>!['id','name','email','password','status','permissions','tenantIds'].includes(k))||['id','name','email','status','permissions','tenantIds'].some(k=>!Object.hasOwn(body,k)))fail('INVALID_INPUT');v.id(body.id);if(body.id===row.id)fail();
 ...
 let passwordUpdated=false;if(Object.hasOwn(body,'password')){const password=validation.password(body.password,'employee');await client.query('UPDATE wpay_auth.credentials SET password_record=$2,temporary_required=false,temporary_expires_at=NULL WHERE account_id=$1',[target.id,await hashPassword(password,'employee')]);passwordUpdated=true;}
 await record(client,row,target.id,version,selected.permissions,selected.tenants,body.status);return {id:target.id,permissionVersion:version,sessionsInvalidated:true,passwordUpdated};
}
