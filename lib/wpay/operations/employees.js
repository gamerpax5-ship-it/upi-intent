"use strict";
const {randomUUID}=require('node:crypto'),validation=require('../auth/runtime/validation'),v=require('../business/validation'),{hashPassword}=require('../auth/runtime/passwords'),{validateGrants}=require('../authorization-policy'),{PERMISSION_CATALOG}=require('../permission-catalog'),{fail,recent,permit,audit}=require('./access');
...
async function run(client,row,context,operation,body){
 ...
 if(operation==='operations/employee/create'){
  validation.exactFields(body,['name','email','password','permissions','tenantIds']);const name=validation.name(body.name),email=validation.email(body.email),password=validation.password(body.password,'employee'),selected=selection(context,body.permissions,body.tenantIds);
  ...
  await client.query('INSERT INTO wpay_auth.credentials(account_id,password_record,temporary_required,temporary_expires_at) VALUES($1,$2,false,NULL)',[id,recordHash]);
  ...
  await record(client,row,id,1,selected.permissions,selected.tenants,'active');return {id,email,passwordConfigured:true,passwordResetRequired:false,mfaRequired:true,loginPath:'/employee'};
 }
