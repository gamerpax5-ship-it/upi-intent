...
async function run(client,row,context,operation,body){
 ...
 if(operation==='operations/employee/create'){
  validation.exactFields(body,['name','email','permissions','tenantIds']);const name=validation.name(body.name),email=validation.email(body.email),selected=selection(context,body.permissions,body.tenantIds);
  permit(context,'employee_management.create',{kind:'create',tenantId:selected.tenants[0]});
  const id=randomUUID(),password='WPay!'+randomBytes(24).toString('base64url'),recordHash=await hashPassword(password);
  ...
  const credential=await client.query("INSERT INTO wpay_auth.credentials(account_id,password_record,temporary_required,temporary_expires_at) VALUES($1,$2,true,CURRENT_TIMESTAMP+interval '24 hours') RETURNING temporary_expires_at",[id,recordHash]);
  ...
  await record(client,row,id,1,selected.permissions,selected.tenants,'active');return {id,email,oneTimePassword:password,passwordResetRequired:true,temporaryExpiresAt:credential.rows[0].temporary_expires_at,mfaRequired:true,loginPath:'/employee'};
 }
