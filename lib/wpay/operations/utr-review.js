...
async function target(c,row,context,body,crypto){
 ...
 const o=(await c.query(`SELECT o.*,r.bank_id,u.user_id AS owner_subject,u.tenant_id AS user_tenant,cl.id AS claim_id,cl.encrypted_utr,cl.utr_digest,f.utr_digest AS verified_digest
 FROM wpay_auth.gateway_claims cl JOIN wpay_auth.gateway_orders o ON o.id=cl.order_id
 JOIN wpay_auth.business_reservations r ON r.id=o.reservation_id JOIN wpay_auth.accounts u ON u.id=r.user_id JOIN wpay_auth.accounts m ON m.id=o.merchant_id LEFT JOIN wpay_auth.business_financial_events f ON f.reservation_id=r.id
...
async function list(c,row,context,body,crypto){
 ...
 if(!Number.isInteger(offset)||offset<0||offset>100000||!['pending','verified','approved','rejected','all'].includes(status))fail('INVALID_INPUT');
 ...
 CASE WHEN o.state='successful' AND cl.utr_digest=f.utr_digest THEN CASE WHEN o.evidence_state='admin_approved' THEN 'approved' ELSE 'verified' END WHEN o.evidence_state='rejected' OR (o.state='failed' AND o.evidence_state='verified') OR o.state IN('cancelled','successful') THEN 'rejected' ELSE 'pending' END AS review_status
 ...
 return {offset,hasMore:rows.length>50,records:rows.slice(0,50).map(o=>{
  ...
  let canApprove=false;try{permit(context,'utr_center.approve',resource(o));canApprove=canReview;}catch{}
  return {canApprove,claimId:o.id,orderId:o.order_id,utr:crypto.open(o.encrypted_utr,binding('claim',o.id)),reference:o.reference,amountMinor:o.amount_minor,merchant:o.merchant_name,user:o.user_name,submittedAt:o.created_at,status:o.review_status,paymentStatus:o.payment_status,canReview};
 })};
}
async function prepare(c,row,context,body,crypto,gateway){
 v.exactFields(body,['claimId','action','reason']);v.reason(body.reason);if(!['verify','approve','reject'].includes(body.action))fail('INVALID_INPUT');
 await ledger.lock(c);const o=await target(c,row,context,body,crypto);
 if(body.action==='approve')permit(context,'utr_center.approve',resource(o));
 if(o.state==='successful'){
  const same=o.utr_digest===o.verified_digest,manual=o.evidence_state==='admin_approved';
  return {status:'successful',reviewStatus:same?(manual?'approved':'verified'):'rejected',final:true,bankVerified:!manual,approvalMethod:manual?'admin_manual':'bank_evidence',message:same?(manual?'Admin approved — payment Successful.':'Verified — payment Successful.'):'Payment already Successful with a different UTR. This claim was not approved.'};
 }
 if(o.evidence_state==='rejected'||o.state==='cancelled'){if(body.action!=='reject')fail('CONFLICT');return {status:'failed',final:true};}
 if(body.action==='approve')return require('./utr-approval').approve(c,row,o,body.reason,gateway);
...
async function finish(c,row,context,body,crypto,result){
 ...
 const manual=o.evidence_state==='admin_approved',success=o.state==='successful'&&['verified','admin_approved'].includes(o.evidence_state)&&o.utr_digest===o.verified_digest,rejected=o.evidence_state==='rejected'||(o.state==='failed'&&o.evidence_state==='verified');
 ...
 return {status:o.state,reviewStatus:success?(manual?'approved':'verified'):rejected?'rejected':'not_verified',final:success||rejected,message:success?(manual?'Admin approved — payment Successful.':'Verified — payment Successful.'):rejected?'Rejected — payment Failed.':'Not verified: matching bank evidence is not available. No payment was approved.'};
}
