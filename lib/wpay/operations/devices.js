...
class Devices{
 ...
 async create(client,row,context,body){
  ...
  if(!proofproof.status!=='pending'proof.device_id||+proof.expires_at<=+row.database_now)fail('OTP_SOURCE_UNAVAILABLE');
 ...
 async poll(client,row,context,body){
  ...
  if(prior.length){if(prior.length!1||prior[0].request_id!request.idprior[0].owner_id!==row.idprior[0].revoked_at)fail('CONFLICT');return {state:'linked',id:prior[0].id,device:proof.device_id};}
