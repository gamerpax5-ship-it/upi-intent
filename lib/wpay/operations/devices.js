...
class Devices{
 ...
 async links(client,row,context,filter={}){
  ...
  const owned=(await client.query(`SELECT d.id,d.owner_id,d.source_id,d.device_ref,d.pairing_id,d.valid_from,d.valid_until,a.name AS owner_name,a.tenant_id FROM wpay_auth.paired_devices d JOIN wpay_auth.accounts a ON a.id=d.owner_id JOIN wpay_auth.eligibility e ON e.account_id=a.id
   WHERE d.revoked_at IS NULL AND d.valid_from<=CURRENT_TIMESTAMP AND d.valid_until>CURRENT_TIMESTAMP AND a.status='active' AND e.approval_status='approved'
   AND ($1::uuid IS NULL OR d.owner_id=$1) AND ($2::text[] IS NULL OR a.tenant_id=ANY($2)) AND ($3::uuid IS NULL OR d.owner_id=$3) AND ($4::text IS NULL OR d.device_ref=$4)
   AND ($5::text IS NULL OR d.device_ref COLLATE "C">$5) ORDER BY d.device_ref COLLATE "C" LIMIT 101 FOR SHARE OF d`,[user?row.id:null,user?null:scope.tenantIds,filter.ownerId??null,filter.device??null,filter.afterDevice??null])).rows;
  // Independently verified, still-current mappings from the previous adapter
  // remain valid authority; a pending browser-submitted device ID never does.
  const prior=(await client.query(`SELECT d.id,d.account_id AS owner_id,d.source_id,d.resource_id AS device_ref,NULL::text AS pairing_id,d.valid_from,d.valid_until,d.verified_at AS authority_at,a.name AS owner_name,a.tenant_id
   FROM wpay_auth.resource_links d JOIN wpay_auth.accounts a ON a.id=d.account_id JOIN wpay_auth.eligibility e ON e.account_id=a.id
   WHERE d.resource_kind='device' AND d.source_id='legacy-primary' AND d.status='verified' AND d.consent_at<=CURRENT_TIMESTAMP AND d.verified_at<=CURRENT_TIMESTAMP
   AND NOT EXISTS(SELECT 1 FROM wpay_auth.paired_devices n WHERE n.source_id=d.source_id AND n.device_ref=d.resource_id)
   AND d.valid_from<=CURRENT_TIMESTAMP AND d.valid_until>CURRENT_TIMESTAMP AND d.revoked_at IS NULL AND a.status='active' AND e.approval_status='approved'
   AND ($1::uuid IS NULL OR d.account_id=$1) AND ($2::text[] IS NULL OR a.tenant_id=ANY($2)) AND ($3::uuid IS NULL OR d.account_id=$3) AND ($4::text IS NULL OR d.resource_id=$4)
   AND ($5::text IS NULL OR d.resource_id COLLATE "C">$5) ORDER BY d.resource_id COLLATE "C" LIMIT 101 FOR SHARE OF d`,[user?row.id:null,user?null:scope.tenantIds,filter.ownerId??null,filter.device??null,filter.afterDevice??null])).rows;
 ...
 async list(client,row,context,body={}){
  ...
  let metadata=[];try{if(links.length&&this.source)metadata=await this.source.devices(links);}catch{}
  return {nextDeviceCursor,sourceConnected:!!this.source&&await this.source.ready(),pairingAvailable:!!this.source&&!!this.bridge,pending,devices:links.map(l=>({id:l.id,legacyMapping:!l.pairing_id,device:l.device_ref,status:metadata.find(d=>d.id===l.device_ref)?.linked?'linked':'source_unavailable_or_unpaired',validFrom:l.valid_from,validUntil:l.valid_until})),message:!links.length?'No linked device':!metadata.length?'OTP source unavailable':null};
 }
 ...
 async events(client,row,context,body){
  fields(body,['before','sender','device','ownerId','reveal','afterDevice']);const before=cursor(body.before),sender=body.sender??'';if(typeof sender!=='string'||sender.length>160||body.reveal!==undefined&&typeof body.reveal!=='boolean')fail('INVALID_INPUT');
