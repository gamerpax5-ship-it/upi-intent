...
return {nextDeviceCursor,sourceConnected,pairingAvailable:pairingStatus==='ready',pairingStatus,pending,devices:links.map(l=>({id:l.id,legacyMapping:!l.pairing_id,device:l.device_ref,status:metadata.find(d=>d.id===l.device_ref)?.linked?'linked':'source_unavailable_or_unpaired',validFrom:l.valid_from,validUntil:l.valid_until})),message:!links.length?'No linked device':!metadata.length?'Device source unavailable':null};
 ...
 async poll(client,row,context,body){
  ...
  const request=(await client.query('SELECT * FROM wpay_auth.legacy_pairing_requests WHERE id=$1 AND owner_id=$2',[v.id(body.requestId),row.id])).rows[0];
  ...
  const old=await client.query("SELECT 1 FROM wpay_auth.resource_links WHERE source_id='legacy-primary' AND resource_kind='device' AND resource_id=$1 AND status='verified' AND revoked_at IS NULL AND valid_until>CURRENT_TIMESTAMP AND account_id<>$2",[proof.device_id,row.id]);
 ...
 async revoke(client,row,context,body){v.exactFields(body,['id']);if(row.account_type!=='user')fail();permit(context,'user.device_pairing.revoke',accountResource(row));recent(row);const r=await client.query('UPDATE wpay_auth.paired_devices SET revoked_at=CURRENT_TIMESTAMP WHERE id=$1 AND owner_id=$2 AND revoked_at IS NULL RETURNING id',[v.id(body.id),row.id]);if(!r.rowCount)fail();await audit(client,row.id,row.id,body.id,'device_ownership_revoked');return {revoked:true};}fied' AND revoked_at IS NULL AND valid_until>CURRENT_TIMESTAMP AND account_id<>$2",[proof.device_id,row.id]);if(!r.rowCount)fail();await audit(client,row.id,row.id,body.id,'device_ownership_revoked');return {revoked:true};}
