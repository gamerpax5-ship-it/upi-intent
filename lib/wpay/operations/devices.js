...
class Devices{
 constructor({source=null,pairingSource=null,bridge=null,crypto}){this.source=source;this.pairingSource=pairingSource||source;this.bridge=bridge;this.crypto=crypto;}
...
const pairingConnected=!!this.pairingSource&&await this.pairingSource.ready();
const pairingStatus=!this.pairingSource||!this.bridge?'not_configured':pairingConnected?'ready':'source_unavailable';
return {nextDeviceCursor,sourceConnected,pairingAvailable:pairingStatus==='ready',pairingStatus,pending,devices:links.map(l=>({id:l.id,legacyMapping:!l.pairing_id,device:l.device_ref,status:metadata.find(d=>d.id===l.device_ref)?.linked?'linked':'source_unavailable_or_unpaired',validFrom:l.valid_from,validUntil:l.valid_until})),message:!links.length?'No linked device':!metadata.length?'Device source unavailable':null};
 ...
 async create(client,row,context,body){
  ...
  if(!this.pairingSource||!this.bridge)fail('OTP_SOURCE_UNAVAILABLE');
  ...
  const code=await this.bridge.issue(),digest=createHash('sha256').update(code).digest('hex'),proof=await this.pairingSource.pairing(digest);
 ...
 async poll(client,row,context,body){
  ...
  const request=(await client.query('SELECT * FROM wpay_auth.legacy_pairing_requests WHERE id=$1 AND owner_id=$2',[v.id(body.requestId),row.id])).rows[0];if(!request)fail();if(!this.pairingSource)fail('OTP_SOURCE_UNAVAILABLE');
  const proof=await this.pairingSource.pairing(request.token_digest);if(!proof||proof.id!==request.pairing_id)fail('OTP_SOURCE_UNAVAILABLE');
