...
class Reconciliation {
 ...
 async observations(client,row,context,body){
  require('../gateway/validation').fields(body,['orderId','offset','status']);const user=row.account_type==='user';
  ...
  const status=body.status??'';if(!['','pending_payment','verification_pending','successful','failed','expired','cancelled','recovery_review'].includes(status))fail('INVALID_INPUT');
  ...
  const rows=(await client.query(`SELECT o.id,o.reference,o.merchant_id,o.amount_minor,o.state,o.evidence_state,o.created_at,o.paid_at,r.user_id,r.bank_id,r.bank_version,
   f.source,f.utr_digest AS verified_digest,f.journal_id,(SELECT bv.details->>'upiId' FROM wpay_auth.business_bank_versions bv WHERE bv.bank_id=r.bank_id AND bv.version=r.bank_version) AS upi_id,
   ...
   AND ($3::uuid IS NULL OR o.id=$3) AND ($5::text='' OR o.state=$5) ORDER BY o.created_at DESC,o.id LIMIT 51 OFFSET $4`,[user?row.id:null,user?null:scope.tenantIds,body.orderId??null,offset,status])).rows;
  if(body.orderId&&!rows.length)fail();
  const records=rows.slice(0,50).map(o=>({orderId:o.id,reference:o.reference,upiId:o.upi_id,amountMinor:o.amount_minor,currency:'INR',createdAt:o.created_at,paidAt:o.paid_at,status:o.state,evidenceState:o.evidence_state,
   ...
   observations:o.claims.map(c=>({claimId:c.id,utr:this.crypto.open(c.value,binding('claim',c.id)),capturedAt:c.at,source:c.digest===o.verified_digest?o.source:'checkout_claim',verified:c.digest===o.verified_digest}))}));
