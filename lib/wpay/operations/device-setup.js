...
function metadata(link,device,now){
 ...
 return {id:link.id,ownerId:link.owner_id,ownerName:link.owner_name,device:link.device_ref,legacyMapping:!link.pairing_id,
  ...
  phone:typeof device?.phone_e164==='string'?device.phone_e164.slice(0,40):null,carrier:typeof device?.sim_carrier==='string'?device.sim_carrier.slice(0,120):null,model:typeof device?.model==='string'?device.model.slice(0,120):null,apkVersion:typeof device?.app_version==='string'?device.app_version:null,
  battery:Number.isFinite(Number(device?.battery_level))?Math.max(0,Math.min(100,Number(device.battery_level))):null,charging:typeof device?.charging==='boolean'?device.charging:null,
  batteryHealth:typeof device?.battery_health==='string'?device.battery_health.slice(0,80):null,
  network:typeof device?.network_type==='string'?device.network_type.slice(0,80):null,simName:typeof device?.sim_name==='string'?device.sim_name.slice(0,120):null,
  latitude:Number.isFinite(Number(device?.latitude))?Number(device.latitude):null,longitude:Number.isFinite(Number(device?.longitude))?Number(device.longitude):null,
  locationPermission:typeof device?.location_permission==='boolean'?device.location_permission:null,locationEnabled:typeof device?.location_enabled==='boolean'?device.location_enabled:null,
  locationLabel:typeof device?.location_label==='string'?device.location_label.slice(0,160):null,validFrom:link.valid_from,validUntil:link.valid_until};
}
class DeviceSetup{
 ...
 async list(c,row,context,body){
  ...
  return {sourceConnected:connected,pairingAvailable:connected&&!!this.devices.bridge,canCreate,setupAllowed,canRevoke:canUsePermission({...context,permissionId:permission(row,'revoke')}).allowed,
   pairingStatus:!source||!this.devices.bridge?'not_configured':connected?'ready':'source_unavailable',diagnosticsAvailable:typeof source?.diagnostics==='function',
 ...
 async history(c,row,context,body){
  ...
  const rows=(await c.query(`SELECT r.id,r.owner_id,a.name owner_name,a.account_type owner_type,r.created_at,r.expires_at,d.device_ref,d.revoked_at,
