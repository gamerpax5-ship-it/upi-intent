...
class DeviceSetup{
 ...
 async revoke(c,row,context,body){
  ...
  const link=(await c.query('SELECT d.*,a.tenant_id,a.account_type,a.user_id FROM wpay_auth.paired_devices d JOIN wpay_auth.accounts a ON a.id=d.owner_id WHERE d.id=$1',[body.id])).rows[0];if(!link||row.account_type==='user'&&link.owner_id!==row.id)fail();
