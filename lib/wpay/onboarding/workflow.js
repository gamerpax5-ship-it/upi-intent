...
class Onboarding {
 ...
 async result(challenge,bank,verificationState){
  return {id:challenge.id,bankId:challenge.bank_id,version:challenge.bank_version,status:challenge.status,amountMinor:String(challenge.amount_minor),currency:'INR',expiresAt:challenge.expires_at,
   attempts:challenge.attempts,synthetic:challenge.synthetic||this.evidence.allowSynthetic,verificationConnected:typeof this.evidence.verify==='function',verificationState:verificationState||(challenge.status==='verified'?'verified':typeof this.evidence.verify==='function'?'pending':'unavailable'),message:challenge.status==='waiting'?(typeof this.evidence.verify!=='function'||verificationState==='unavailable'?'Payment verification source unavailable':'Waiting for verified payment evidence'):challenge.status,
 ...
 async challenge(client,owner,input,cancel=false){
  ...
  let observation;try{observation=await this.evidence.inspect(challenge);}catch{observation={state:'unavailable',proof:null};}const proof=observation.proof;
  if(+challenge.expires_at<=Date.now()){await this.close(client,challenge,'expired');return this.result(challenge,bank);}
  if(proof){
   ...
   }else observation.state='duplicate_evidence';
  }
  return this.result(challenge,bank,observation.state);
 }
 ...
 async analytics(client,owner){
  const rows=(await client.query(`SELECT b.id,b.version,b.status,v.details->>'upiId' AS upi_id,
   count(r.id)::integer AS total,count(r.id) FILTER(WHERE f.reservation_id IS NOT NULL OR g.state='successful')::integer AS successful,
   count(r.id) FILTER(WHERE f.reservation_id IS NULL AND (g.state='failed' OR g.id IS NULL AND o.reservation_id IS NOT NULL))::integer AS failed,
   count(r.id) FILTER(WHERE f.reservation_id IS NULL AND (g.state IN('pending_payment','verification_pending','recovery_review') OR g.id IS NULL AND o.reservation_id IS NULL AND r.state='active' AND r.expires_at>clock_timestamp()))::integer AS pending,
   count(r.id) FILTER(WHERE f.reservation_id IS NULL AND (g.state='expired' OR g.id IS NULL AND o.reservation_id IS NULL AND (r.state='expired' OR r.state='active' AND r.expires_at<=clock_timestamp())))::integer AS expired,
   count(r.id) FILTER(WHERE f.reservation_id IS NULL AND (g.state='cancelled' OR g.id IS NULL AND o.reservation_id IS NULL AND r.state IN('cancelled','released')))::integer AS cancelled,
   COALESCE(sum(COALESCE(f.amount_minor,CASE WHEN g.state='successful' THEN g.amount_minor END)),0)::text AS successful_volume_minor
   ...
   LEFT JOIN wpay_auth.gateway_orders g ON g.reservation_id=r.id
