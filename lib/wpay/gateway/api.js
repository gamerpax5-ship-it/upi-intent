...
const GET=['gateway/summary','gateway/keys','gateway/webhooks','gateway/orders','gateway/logs','gateway/diagnostics'],POST=['gateway/analytics','gateway/route-status','gateway/create','gateway/get','gateway/search','gateway/keys/create','gateway/keys/revoke','gateway/webhooks/configure','gateway/webhooks/retry'];
...
async function run(gateway,client,row,context,operation,body){
 ...
 switch(operation){
  case 'gateway/analytics':return require('./merchant-analytics').analytics(client,row.id,body);
  case 'gateway/route-status':{v.exactFields(body,['amountMinor']);const amount=require('../business/money').minor(body.amountMinor).toString(),candidates=await gateway.core.candidates(client,row.id),checks=candidates.map(c=>require('../business/routing').eligibility(c,amount,c.databaseNow));return {eligible:checks.some(c=>c.eligible),reasons:checks.some(c=>c.eligible)?[]:candidates.length?[...new Set(checks.flatMap(c=>c.reasons))]:['no_assignment']};}
