...
const GET=['gateway/summary','gateway/keys','gateway/webhooks','gateway/orders','gateway/diagnostics'],POST=['gateway/create','gateway/get','gateway/search','gateway/keys/create','gateway/keys/revoke','gateway/webhooks/configure','gateway/webhooks/retry'];
...
async function run(gateway,client,row,context,operation,body){
 ...
 if(manage&&(!row.mfa_at||+new Date(row.database_now)-+new Date(row.mfa_at)>300000))throw new AuthError('RECENT_MFA_REQUIRED');
 ...
 switch(operation){
  ...
  case 'gateway/get':v.exactFields(body,['id']);return gateway.get(client,row.id,body.id,gateway.origin);
