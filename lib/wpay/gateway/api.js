...
const GET=['gateway/summary','gateway/keys','gateway/webhooks','gateway/orders','gateway/logs','gateway/diagnostics'],POST=['gateway/admin/approve','gateway/admin/callback','gateway/create','gateway/get','gateway/search','gateway/keys/create','gateway/keys/revoke','gateway/webhooks/configure','gateway/webhooks/retry'];
...
async function run(gateway,client,row,context,operation,body){
 ...
 if(operation.startsWith('gateway/admin/'))return require('./admin-actions').run(gateway,client,row,context,operation,body);
 ...
 if(!merchant){
  ...
  if(!['gateway/orders','gateway/search'].includes(operation))throw new AuthError('FORBIDDEN');const result=await gateway.list(client,null,body,permit.constraints.where.tenantIds);return {...result,canApprove:canUsePermission({...context,permissionId:'payment_admin.approve'}).allowed,canCallback:canUsePermission({...context,permissionId:'payment_admin.callback'}).allowed};
 }
