...
function order(body){fields(body,['reference','idempotencyKey','amountMinor','currency','description','metadata','ttlSeconds']);v.reference(body.reference);v.reference(body.idempotencyKey);
 if(body.currency!=='INR'||money.minor(body.amountMinor)>money.MAX)bad();
 const ttlSeconds=body.ttlSeconds??300;if(!Number.isInteger(ttlSeconds)||ttlSeconds<30||ttlSeconds>900)bad();
 ...
 return {reference:body.reference,idempotencyKey:body.idempotencyKey,amountMinor:body.amountMinor,currency:'INR',description:text(body.description??''),metadata:canonical(metadata),ttlSeconds};
}
