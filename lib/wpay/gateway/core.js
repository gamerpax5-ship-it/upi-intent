...
class Gateway {
 ...
 async merchant(client,id){
  ...
  this.crypto.open(row.encrypted_secret,`wpay-factor:${row.id}:${row.factor_version}`);
 ...
 async create(client,merchantId,body,origin='manual',base){
  ...
  const id=randomUUID(),reserved=await this.core.reserve(client,merchantId,{orderReference:'gw:'+id,idempotencyKey:'gw:'+id,amountMinor:value.amountMinor,ttlSeconds:value.ttlSeconds});
 ...
 async summary(client,id){
  ...
  const balance=await ledger.summary(client,id);return {counts,total:rows.reduce((s,r)=>s+r.count,0),successRate:success+failed?success/(success+failed):null,successfulVolumeMinor:rows.find(r=>r.state==='successful')?.volume||'0',currency:'INR',gross:balance.gross,fees:balance.fees,held:balance.merchantHeld,available:balance.merchantAvailable,verificationConnected:typeof this.verifier==='function',synthetic:this.allowSynthetic};
 }
 async emit(client,row,type){
  const id=randomUUID(),body=JSON.stringify({eventId:id,eventType:type,version:1,orderId:row.id,merchantReference:row.reference,amountMinor:row.amount_minor,currency:'INR',status:row.state,createdAt:row.created_at,paidAt:row.paid_at,expiresAt:row.expires_at});
  await client.query(`INSERT INTO wpay_auth.gateway_outbox(id,order_id,merchant_id,endpoint_id,event_type,body,state) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(order_id,event_type) DO NOTHING`,[id,row.id,row.merchant_id,row.endpoint_id,type,body,row.endpoint_id?'pending':'unconfigured']);
 }
 ...
 async createKey(client,row,body){input.fields(body,['label','scopes','rotateId']);const label=input.text(body.label,60);if(!label||!Array.isArray(body.scopes)||!body.scopes.length||body.scopes.length>2||body.scopes.some(s=>!['orders:read','orders:write'].includes(s)))fail('INVALID_INPUT');
  ...
  await client.query('INSERT INTO wpay_auth.gateway_keys(id,merchant_id,prefix,digest,label,scopes,security_version,factor_version) VALUES($1,$2,$3,$4,$5,$6,$7,$8)',[id,row.id,prefix,hash(secret),label,[...new Set(body.scopes)],row.security_version,row.factor_version]);await ledger.audit(client,{actorId:row.id,ownerId:row.id,entityId:id,event:'gateway_key_created'});return {id,prefix,secret,displayOnce:true};
 }
 ...
 async api(secret,scope,fn){
  ...
  const allowed=await transaction(this.pool,async client=>{const {key}=await this.apiKey(client,secret,scope);return (await client.query(`INSERT INTO wpay_auth.gateway_rate(key_id,hits) VALUES($1,1) ON CONFLICT(key_id) DO UPDATE SET hits=CASE WHEN gateway_rate.window_at<CURRENT_TIMESTAMP-interval '1 minute' THEN 1 ELSE gateway_rate.hits+1 END,window_at=CASE WHEN gateway_rate.window_at<CURRENT_TIMESTAMP-interval '1 minute' THEN CURRENT_TIMESTAMP ELSE gateway_rate.window_at END RETURNING hits`,[key.id])).rows[0].hits<=60;});if(!allowed)fail('RATE_LIMITED');
