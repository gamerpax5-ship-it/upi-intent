...
class Gateway {
 ...
 async create(client,merchantId,body,origin='manual',base){
  ...
  const endpoint=(await client.query(`SELECT e.id FROM wpay_auth.gateway_endpoints e LEFT JOIN wpay_auth.gateway_endpoint_controls c ON c.merchant_id=e.merchant_id WHERE e.merchant_id=$1 AND COALESCE(c.enabled,true)=true ORDER BY e.created_at DESC,e.id DESC LIMIT 1`,[merchantId])).rows[0];
 ...
 async summary(client,id){
  ...
  const frozen=(await client.query("SELECT COALESCE(sum(amount_minor),0)::text AS amount FROM wpay_auth.business_holds WHERE owner_id=$1 AND state='active' AND category='frozen'",[id])).rows[0].amount;
  return {frozenMinor:frozen,counts,total:rows.reduce((s,r)=>s+r.count,0),successRate:success+failed?success/(success+failed):null,successfulVolumeMinor:rows.find(r=>r.state==='successful')?.volume||'0',currency:'INR',gross:balance.gross,fees:balance.fees,held:balance.merchantHeld,available:balance.merchantAvailable,linkTtlSeconds:Number.isInteger(ttl)?ttl:300,commercialVersion:terms?.version??null,verificationConnected:typeof this.verifier==='function',synthetic:this.allowSynthetic};
 }
 async emit(client,row,type,resource='order',eventKey=row.id){
  if(!['order','payout'].includes(resource))fail('INVALID_INPUT');
  const id=randomUUID(),payout=resource==='payout',body=JSON.stringify({eventId:id,eventType:type,version:1,...(payout?{payoutId:row.id}:{orderId:row.id}),merchantReference:row.reference,amountMinor:row.amount_minor,currency:'INR',status:row.state,createdAt:row.created_at,...(payout?{completedAt:row.completed_at}:{paidAt:row.paid_at,expiresAt:row.expires_at,...(row.evidence_state==='admin_approved'?{approvalMethod:'admin_manual',bankVerified:false}:{})})});
 ...
 async expireOne(client,row){
  const claimed=(await client.query('SELECT 1 FROM wpay_auth.gateway_claims WHERE order_id=$1 LIMIT 1',[row.id])).rowCount>0;
  const state=claimed?'failed':'expired';
  if(row.state===state)return;
  await this.core.release(client,row.reservation_id,null,'expired');
  row.state=state;
  await client.query('UPDATE wpay_auth.gateway_orders SET state=$2 WHERE id=$1',[row.id,state]);
  await this.emit(client,row,'payment.'+state);
 }
 async expire(client){
  await ledger.lock(client);
  const rows=(await client.query("SELECT o.* FROM wpay_auth.gateway_orders o WHERE (state IN('pending_payment','verification_pending','recovery_review') AND expires_at<=CURRENT_TIMESTAMP) OR (state='expired' AND EXISTS(SELECT 1 FROM wpay_auth.gateway_claims c WHERE c.order_id=o.id)) ORDER BY expires_at,id LIMIT 200 FOR UPDATE OF o")).rows;
  for(const row of rows)await this.expireOne(client,row);return rows.length;
 }
 async customer(client,secret,utr){
  ...
  if((['pending_payment','verification_pending','recovery_review'].includes(row.state)&&+row.expires_at<=Date.now())||row.state==='expired')await this.expireOne(client,row);
  if(utr!==undefined){
 ...
 async configure(client,merchantId,body,actorId=merchantId){v.exactFields(body,['url']);webhooks.endpoint(body.url,{testCallback:this.testCallback});const id=randomUUID(),secret=token();
  await client.query('INSERT INTO wpay_auth.gateway_endpoints(id,merchant_id,url,encrypted_secret) VALUES($1,$2,$3,$4)',[id,merchantId,body.url,this.crypto.seal(secret,binding('webhook',id))]);await client.query(`INSERT INTO wpay_auth.gateway_endpoint_controls(merchant_id,enabled,actor_id,updated_at) VALUES($1,true,$2,CURRENT_TIMESTAMP) ON CONFLICT(merchant_id) DO UPDATE SET enabled=true,actor_id=EXCLUDED.actor_id,updated_at=CURRENT_TIMESTAMP`,[merchantId,actorId]);await ledger.audit(client,{actorId,ownerId:merchantId,entityId:id,event:'gateway_webhook_rotated'});return {id,url:body.url,secret,displayOnce:true,enabled:true};
 }
 async dispatch(){
  await this.pool.query("UPDATE wpay_auth.gateway_outbox SET state='failed',lease_id=NULL,lease_until=NULL WHERE state='leased' AND attempts>=8 AND lease_until<=CURRENT_TIMESTAMP");
  const event=await transaction(this.pool,async c=>(await c.query(`WITH due AS(SELECT id FROM wpay_auth.gateway_outbox WHERE attempts<8 AND ((state='pending' AND next_attempt_at<=CURRENT_TIMESTAMP) OR (state='leased' AND lease_until<=CURRENT_TIMESTAMP)) ORDER BY next_attempt_at,id FOR UPDATE SKIP LOCKED LIMIT 1) UPDATE wpay_auth.gateway_outbox o SET state='leased',attempts=attempts+1,lease_id=$1,lease_until=CURRENT_TIMESTAMP+interval '30 seconds' FROM due WHERE o.id=due.id RETURNING o.*`,[randomUUID()])).rows[0]);if(!event)return false;
 ...
 async verifyOrder(id,{expectedUtr}={}){
  ...
  const snapshot=await transaction(this.pool,async c=>{const o=(await c.query('SELECT * FROM wpay_auth.gateway_orders WHERE id=$1',[v.id(id)])).rows[0];if(!o)fail('NOT_FOUND');const r=(await c.query('SELECT * FROM wpay_auth.business_reservations WHERE id=$1',[o.reservation_id])).rows[0],bank=(await c.query('SELECT details FROM wpay_auth.business_bank_versions WHERE bank_id=$1 AND version=$2',[r.bank_id,r.bank_version])).rows[0];
   ...
   if(o.evidence_state==='rejected')return {rejected:true};
   if(expectedUtr&&!claims.includes(expectedUtr))fail('CONFLICT');
   return {orderId:o.id,reservationId:r.id,merchantId:r.merchant_id,userId:r.user_id,bankId:r.bank_id,bankVersion:r.bank_version,upi:bank.details.upiId,accountDigest:hash(bank.details.ifsc+':'+bank.details.accountNumber),amountMinor:r.amount_minor,currency:'INR',createdAt:o.created_at.toISOString(),expiresAt:o.expires_at.toISOString(),claims:expectedUtr?[expectedUtr]:claims};});
  if(snapshot.rejected)return {status:'failed'};
  ...
  if(expectedUtr&&proof.utr!==expectedUtr)return {status:'not_verified'};
  ...
  return transaction(this.pool,async c=>{await ledger.lock(c);const row=(await c.query('SELECT * FROM wpay_auth.gateway_orders WHERE id=$1 FOR UPDATE',[id])).rows[0];if(row.state==='successful')return {status:'successful',alreadyAccounted:true};
   if(row.evidence_state==='rejected')return {status:'failed'};
   ...
   if(proof.status==='failed'){
    if(!['pending_payment','verification_pending','recovery_review','expired'].includes(row.state))return {status:row.state};await this.core.release(c,row.reservation_id,null,'released');
