...
const ACCOUNT = `SELECT a.*,e.approval_status,c.password_record,c.temporary_required,c.temporary_expires_at,z.security_version,z.factor_version,z.enabled AS mfa_enabled,
...
class SecurityRepository extends AuthRepository {
  ...
  async startAdminSession(accountId, expectedPassword, action) {
    return transaction(this.pool,async client=>{
      const row=await this.lockedAccount(client,accountId);
      if(credentialFingerprint(row.password_record)!==credentialFingerprint(expectedPassword))throw new AuthError('AUTH_FAILED');
      return action(client,row);
  ...
  async promote(client, row, sessionDigest, validate, verifiedAt = row.database_now) {
    if (!approvalAllows(row) || row.temporary_required || !row.mfa_enabled || !row.factor_version) throw new AuthError("MFA_FAILED");
  ...
  async securitySummary(client, row, currentDigest) {
    const sessions = await client.query(`SELECT created_at AS "createdAt",last_seen_at AS "lastSeenAt",expires_at AS "expiresAt",token_digest=$2 AS current
      ...
      AND last_seen_at>CURRENT_TIMESTAMP-interval '30 minutes' AND session_epoch=$3 AND security_version=$4 AND factor_version=$5 AND (mfa_at IS NOT NULL OR (auth_method='password' AND password_at IS NOT NULL))
