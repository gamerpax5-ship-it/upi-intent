...
const SNAPSHOT = `SELECT a.id,a.subject_id,a.tenant_id,a.account_type,a.status,a.user_id,a.merchant_id,
  ...
  s.mfa_at,s.security_version AS session_security_version,s.factor_version AS session_factor_version,
...
class AuthRepository {
  ...
  async throttle(operation, ip) {
    ...
    if (!Object.hasOwn(limits, operation) || !["127.0.0.1", "::ffff:127.0.0.1"].includes(ip)) throw new AuthError("FORBIDDEN");
    const [limit, seconds] = limits[operation];
    const bucket = operation + ":" + createHash("sha256").update("loopback").digest("hex");
  ...
  async createAccount(input, passwordRecord, permissions, bootstrap = false) {
    return transaction(this.pool, async client => {
      ...
      await client.query("INSERT INTO wpay_auth.grants(account_id,permission_version,permissions,admin_scope) VALUES ($1,1,$2,$3)",
        [id, permissions, bootstrap ? JSON.stringify({ tenantIds: [TENANT] }) : null]);
  ...
  async withSession(digest, action) {
    return transaction(this.pool, async client => {
      ...
      // writer. Serialization failures deny access; they never reuse old data.
