...
class FundingApi{
 ...
 async run(c,row,context,operation,body){const user=row.account_type==='user',admin=['admin','super_admin','employee'].includes(row.account_type);if(!user&&!admin)throw new AuthError('FORBIDDEN');
  ...
  if(user){if(request.owner_id!==row.id)throw new AuthError('FORBIDDEN');permit(context,'user.deposits.submit',{...resource,kind:'create'});}else permit(context,operation==='funding/review'?(body.action==='reject'?'deposits.reject':'deposits.approve'):'deposits.review',resource);
  ...
  if(operation==='funding/review'){if(!admin)throw new AuthError('FORBIDDEN');return this.workflow.review(c,request,body,row.id);}
